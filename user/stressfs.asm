
user/_stressfs:     file format elf64-littleriscv


Disassembly of section .text:

0000000000000000 <main>:
#include "kernel/fs.h"
#include "kernel/fcntl.h"

int
main(int argc, char *argv[])
{
       0:	dc010113          	addi	sp,sp,-576
       4:	22113c23          	sd	ra,568(sp)
       8:	22813823          	sd	s0,560(sp)
       c:	0480                	addi	s0,sp,576
       e:	87aa                	mv	a5,a0
      10:	dcb43023          	sd	a1,-576(s0)
      14:	dcf42623          	sw	a5,-564(s0)
  int fd, i;
  char path[] = "stressfs0";
      18:	00002797          	auipc	a5,0x2
      1c:	14078793          	addi	a5,a5,320 # 2158 <schedule_edf_cbs+0x502>
      20:	6398                	ld	a4,0(a5)
      22:	fce43c23          	sd	a4,-40(s0)
      26:	0087d783          	lhu	a5,8(a5)
      2a:	fef41023          	sh	a5,-32(s0)
  char data[512];

  printf("stressfs starting\n");
      2e:	00002517          	auipc	a0,0x2
      32:	0fa50513          	addi	a0,a0,250 # 2128 <schedule_edf_cbs+0x4d2>
      36:	00001097          	auipc	ra,0x1
      3a:	b2a080e7          	jalr	-1238(ra) # b60 <printf>
  memset(data, 'a', sizeof(data));
      3e:	dd840793          	addi	a5,s0,-552
      42:	20000613          	li	a2,512
      46:	06100593          	li	a1,97
      4a:	853e                	mv	a0,a5
      4c:	00000097          	auipc	ra,0x0
      50:	224080e7          	jalr	548(ra) # 270 <memset>

  for(i = 0; i < 4; i++)
      54:	fe042623          	sw	zero,-20(s0)
      58:	a829                	j	72 <main+0x72>
    if(fork() > 0)
      5a:	00000097          	auipc	ra,0x0
      5e:	5b8080e7          	jalr	1464(ra) # 612 <fork>
      62:	87aa                	mv	a5,a0
      64:	00f04f63          	bgtz	a5,82 <main+0x82>
  for(i = 0; i < 4; i++)
      68:	fec42783          	lw	a5,-20(s0)
      6c:	2785                	addiw	a5,a5,1
      6e:	fef42623          	sw	a5,-20(s0)
      72:	fec42783          	lw	a5,-20(s0)
      76:	0007871b          	sext.w	a4,a5
      7a:	478d                	li	a5,3
      7c:	fce7dfe3          	bge	a5,a4,5a <main+0x5a>
      80:	a011                	j	84 <main+0x84>
      break;
      82:	0001                	nop

  printf("write %d\n", i);
      84:	fec42783          	lw	a5,-20(s0)
      88:	85be                	mv	a1,a5
      8a:	00002517          	auipc	a0,0x2
      8e:	0b650513          	addi	a0,a0,182 # 2140 <schedule_edf_cbs+0x4ea>
      92:	00001097          	auipc	ra,0x1
      96:	ace080e7          	jalr	-1330(ra) # b60 <printf>

  path[8] += i;
      9a:	fe044703          	lbu	a4,-32(s0)
      9e:	fec42783          	lw	a5,-20(s0)
      a2:	0ff7f793          	andi	a5,a5,255
      a6:	9fb9                	addw	a5,a5,a4
      a8:	0ff7f793          	andi	a5,a5,255
      ac:	fef40023          	sb	a5,-32(s0)
  fd = open(path, O_CREATE | O_RDWR);
      b0:	fd840793          	addi	a5,s0,-40
      b4:	20200593          	li	a1,514
      b8:	853e                	mv	a0,a5
      ba:	00000097          	auipc	ra,0x0
      be:	5a0080e7          	jalr	1440(ra) # 65a <open>
      c2:	87aa                	mv	a5,a0
      c4:	fef42423          	sw	a5,-24(s0)
  for(i = 0; i < 20; i++)
      c8:	fe042623          	sw	zero,-20(s0)
      cc:	a015                	j	f0 <main+0xf0>
//    printf(fd, "%d\n", i);
    write(fd, data, sizeof(data));
      ce:	dd840713          	addi	a4,s0,-552
      d2:	fe842783          	lw	a5,-24(s0)
      d6:	20000613          	li	a2,512
      da:	85ba                	mv	a1,a4
      dc:	853e                	mv	a0,a5
      de:	00000097          	auipc	ra,0x0
      e2:	55c080e7          	jalr	1372(ra) # 63a <write>
  for(i = 0; i < 20; i++)
      e6:	fec42783          	lw	a5,-20(s0)
      ea:	2785                	addiw	a5,a5,1
      ec:	fef42623          	sw	a5,-20(s0)
      f0:	fec42783          	lw	a5,-20(s0)
      f4:	0007871b          	sext.w	a4,a5
      f8:	47cd                	li	a5,19
      fa:	fce7dae3          	bge	a5,a4,ce <main+0xce>
  close(fd);
      fe:	fe842783          	lw	a5,-24(s0)
     102:	853e                	mv	a0,a5
     104:	00000097          	auipc	ra,0x0
     108:	53e080e7          	jalr	1342(ra) # 642 <close>

  printf("read\n");
     10c:	00002517          	auipc	a0,0x2
     110:	04450513          	addi	a0,a0,68 # 2150 <schedule_edf_cbs+0x4fa>
     114:	00001097          	auipc	ra,0x1
     118:	a4c080e7          	jalr	-1460(ra) # b60 <printf>

  fd = open(path, O_RDONLY);
     11c:	fd840793          	addi	a5,s0,-40
     120:	4581                	li	a1,0
     122:	853e                	mv	a0,a5
     124:	00000097          	auipc	ra,0x0
     128:	536080e7          	jalr	1334(ra) # 65a <open>
     12c:	87aa                	mv	a5,a0
     12e:	fef42423          	sw	a5,-24(s0)
  for (i = 0; i < 20; i++)
     132:	fe042623          	sw	zero,-20(s0)
     136:	a015                	j	15a <main+0x15a>
    read(fd, data, sizeof(data));
     138:	dd840713          	addi	a4,s0,-552
     13c:	fe842783          	lw	a5,-24(s0)
     140:	20000613          	li	a2,512
     144:	85ba                	mv	a1,a4
     146:	853e                	mv	a0,a5
     148:	00000097          	auipc	ra,0x0
     14c:	4ea080e7          	jalr	1258(ra) # 632 <read>
  for (i = 0; i < 20; i++)
     150:	fec42783          	lw	a5,-20(s0)
     154:	2785                	addiw	a5,a5,1
     156:	fef42623          	sw	a5,-20(s0)
     15a:	fec42783          	lw	a5,-20(s0)
     15e:	0007871b          	sext.w	a4,a5
     162:	47cd                	li	a5,19
     164:	fce7dae3          	bge	a5,a4,138 <main+0x138>
  close(fd);
     168:	fe842783          	lw	a5,-24(s0)
     16c:	853e                	mv	a0,a5
     16e:	00000097          	auipc	ra,0x0
     172:	4d4080e7          	jalr	1236(ra) # 642 <close>

  wait(0);
     176:	4501                	li	a0,0
     178:	00000097          	auipc	ra,0x0
     17c:	4aa080e7          	jalr	1194(ra) # 622 <wait>

  exit(0);
     180:	4501                	li	a0,0
     182:	00000097          	auipc	ra,0x0
     186:	498080e7          	jalr	1176(ra) # 61a <exit>

000000000000018a <strcpy>:
#include "kernel/fcntl.h"
#include "user/user.h"

char*
strcpy(char *s, const char *t)
{
     18a:	7179                	addi	sp,sp,-48
     18c:	f422                	sd	s0,40(sp)
     18e:	1800                	addi	s0,sp,48
     190:	fca43c23          	sd	a0,-40(s0)
     194:	fcb43823          	sd	a1,-48(s0)
  char *os;

  os = s;
     198:	fd843783          	ld	a5,-40(s0)
     19c:	fef43423          	sd	a5,-24(s0)
  while((*s++ = *t++) != 0)
     1a0:	0001                	nop
     1a2:	fd043703          	ld	a4,-48(s0)
     1a6:	00170793          	addi	a5,a4,1
     1aa:	fcf43823          	sd	a5,-48(s0)
     1ae:	fd843783          	ld	a5,-40(s0)
     1b2:	00178693          	addi	a3,a5,1
     1b6:	fcd43c23          	sd	a3,-40(s0)
     1ba:	00074703          	lbu	a4,0(a4)
     1be:	00e78023          	sb	a4,0(a5)
     1c2:	0007c783          	lbu	a5,0(a5)
     1c6:	fff1                	bnez	a5,1a2 <strcpy+0x18>
    ;
  return os;
     1c8:	fe843783          	ld	a5,-24(s0)
}
     1cc:	853e                	mv	a0,a5
     1ce:	7422                	ld	s0,40(sp)
     1d0:	6145                	addi	sp,sp,48
     1d2:	8082                	ret

00000000000001d4 <strcmp>:

int
strcmp(const char *p, const char *q)
{
     1d4:	1101                	addi	sp,sp,-32
     1d6:	ec22                	sd	s0,24(sp)
     1d8:	1000                	addi	s0,sp,32
     1da:	fea43423          	sd	a0,-24(s0)
     1de:	feb43023          	sd	a1,-32(s0)
  while(*p && *p == *q)
     1e2:	a819                	j	1f8 <strcmp+0x24>
    p++, q++;
     1e4:	fe843783          	ld	a5,-24(s0)
     1e8:	0785                	addi	a5,a5,1
     1ea:	fef43423          	sd	a5,-24(s0)
     1ee:	fe043783          	ld	a5,-32(s0)
     1f2:	0785                	addi	a5,a5,1
     1f4:	fef43023          	sd	a5,-32(s0)
  while(*p && *p == *q)
     1f8:	fe843783          	ld	a5,-24(s0)
     1fc:	0007c783          	lbu	a5,0(a5)
     200:	cb99                	beqz	a5,216 <strcmp+0x42>
     202:	fe843783          	ld	a5,-24(s0)
     206:	0007c703          	lbu	a4,0(a5)
     20a:	fe043783          	ld	a5,-32(s0)
     20e:	0007c783          	lbu	a5,0(a5)
     212:	fcf709e3          	beq	a4,a5,1e4 <strcmp+0x10>
  return (uchar)*p - (uchar)*q;
     216:	fe843783          	ld	a5,-24(s0)
     21a:	0007c783          	lbu	a5,0(a5)
     21e:	0007871b          	sext.w	a4,a5
     222:	fe043783          	ld	a5,-32(s0)
     226:	0007c783          	lbu	a5,0(a5)
     22a:	2781                	sext.w	a5,a5
     22c:	40f707bb          	subw	a5,a4,a5
     230:	2781                	sext.w	a5,a5
}
     232:	853e                	mv	a0,a5
     234:	6462                	ld	s0,24(sp)
     236:	6105                	addi	sp,sp,32
     238:	8082                	ret

000000000000023a <strlen>:

uint
strlen(const char *s)
{
     23a:	7179                	addi	sp,sp,-48
     23c:	f422                	sd	s0,40(sp)
     23e:	1800                	addi	s0,sp,48
     240:	fca43c23          	sd	a0,-40(s0)
  int n;

  for(n = 0; s[n]; n++)
     244:	fe042623          	sw	zero,-20(s0)
     248:	a031                	j	254 <strlen+0x1a>
     24a:	fec42783          	lw	a5,-20(s0)
     24e:	2785                	addiw	a5,a5,1
     250:	fef42623          	sw	a5,-20(s0)
     254:	fec42783          	lw	a5,-20(s0)
     258:	fd843703          	ld	a4,-40(s0)
     25c:	97ba                	add	a5,a5,a4
     25e:	0007c783          	lbu	a5,0(a5)
     262:	f7e5                	bnez	a5,24a <strlen+0x10>
    ;
  return n;
     264:	fec42783          	lw	a5,-20(s0)
}
     268:	853e                	mv	a0,a5
     26a:	7422                	ld	s0,40(sp)
     26c:	6145                	addi	sp,sp,48
     26e:	8082                	ret

0000000000000270 <memset>:

void*
memset(void *dst, int c, uint n)
{
     270:	7179                	addi	sp,sp,-48
     272:	f422                	sd	s0,40(sp)
     274:	1800                	addi	s0,sp,48
     276:	fca43c23          	sd	a0,-40(s0)
     27a:	87ae                	mv	a5,a1
     27c:	8732                	mv	a4,a2
     27e:	fcf42a23          	sw	a5,-44(s0)
     282:	87ba                	mv	a5,a4
     284:	fcf42823          	sw	a5,-48(s0)
  char *cdst = (char *) dst;
     288:	fd843783          	ld	a5,-40(s0)
     28c:	fef43023          	sd	a5,-32(s0)
  int i;
  for(i = 0; i < n; i++){
     290:	fe042623          	sw	zero,-20(s0)
     294:	a00d                	j	2b6 <memset+0x46>
    cdst[i] = c;
     296:	fec42783          	lw	a5,-20(s0)
     29a:	fe043703          	ld	a4,-32(s0)
     29e:	97ba                	add	a5,a5,a4
     2a0:	fd442703          	lw	a4,-44(s0)
     2a4:	0ff77713          	andi	a4,a4,255
     2a8:	00e78023          	sb	a4,0(a5)
  for(i = 0; i < n; i++){
     2ac:	fec42783          	lw	a5,-20(s0)
     2b0:	2785                	addiw	a5,a5,1
     2b2:	fef42623          	sw	a5,-20(s0)
     2b6:	fec42703          	lw	a4,-20(s0)
     2ba:	fd042783          	lw	a5,-48(s0)
     2be:	2781                	sext.w	a5,a5
     2c0:	fcf76be3          	bltu	a4,a5,296 <memset+0x26>
  }
  return dst;
     2c4:	fd843783          	ld	a5,-40(s0)
}
     2c8:	853e                	mv	a0,a5
     2ca:	7422                	ld	s0,40(sp)
     2cc:	6145                	addi	sp,sp,48
     2ce:	8082                	ret

00000000000002d0 <strchr>:

char*
strchr(const char *s, char c)
{
     2d0:	1101                	addi	sp,sp,-32
     2d2:	ec22                	sd	s0,24(sp)
     2d4:	1000                	addi	s0,sp,32
     2d6:	fea43423          	sd	a0,-24(s0)
     2da:	87ae                	mv	a5,a1
     2dc:	fef403a3          	sb	a5,-25(s0)
  for(; *s; s++)
     2e0:	a01d                	j	306 <strchr+0x36>
    if(*s == c)
     2e2:	fe843783          	ld	a5,-24(s0)
     2e6:	0007c703          	lbu	a4,0(a5)
     2ea:	fe744783          	lbu	a5,-25(s0)
     2ee:	0ff7f793          	andi	a5,a5,255
     2f2:	00e79563          	bne	a5,a4,2fc <strchr+0x2c>
      return (char*)s;
     2f6:	fe843783          	ld	a5,-24(s0)
     2fa:	a821                	j	312 <strchr+0x42>
  for(; *s; s++)
     2fc:	fe843783          	ld	a5,-24(s0)
     300:	0785                	addi	a5,a5,1
     302:	fef43423          	sd	a5,-24(s0)
     306:	fe843783          	ld	a5,-24(s0)
     30a:	0007c783          	lbu	a5,0(a5)
     30e:	fbf1                	bnez	a5,2e2 <strchr+0x12>
  return 0;
     310:	4781                	li	a5,0
}
     312:	853e                	mv	a0,a5
     314:	6462                	ld	s0,24(sp)
     316:	6105                	addi	sp,sp,32
     318:	8082                	ret

000000000000031a <gets>:

char*
gets(char *buf, int max)
{
     31a:	7179                	addi	sp,sp,-48
     31c:	f406                	sd	ra,40(sp)
     31e:	f022                	sd	s0,32(sp)
     320:	1800                	addi	s0,sp,48
     322:	fca43c23          	sd	a0,-40(s0)
     326:	87ae                	mv	a5,a1
     328:	fcf42a23          	sw	a5,-44(s0)
  int i, cc;
  char c;

  for(i=0; i+1 < max; ){
     32c:	fe042623          	sw	zero,-20(s0)
     330:	a8a1                	j	388 <gets+0x6e>
    cc = read(0, &c, 1);
     332:	fe740793          	addi	a5,s0,-25
     336:	4605                	li	a2,1
     338:	85be                	mv	a1,a5
     33a:	4501                	li	a0,0
     33c:	00000097          	auipc	ra,0x0
     340:	2f6080e7          	jalr	758(ra) # 632 <read>
     344:	87aa                	mv	a5,a0
     346:	fef42423          	sw	a5,-24(s0)
    if(cc < 1)
     34a:	fe842783          	lw	a5,-24(s0)
     34e:	2781                	sext.w	a5,a5
     350:	04f05763          	blez	a5,39e <gets+0x84>
      break;
    buf[i++] = c;
     354:	fec42783          	lw	a5,-20(s0)
     358:	0017871b          	addiw	a4,a5,1
     35c:	fee42623          	sw	a4,-20(s0)
     360:	873e                	mv	a4,a5
     362:	fd843783          	ld	a5,-40(s0)
     366:	97ba                	add	a5,a5,a4
     368:	fe744703          	lbu	a4,-25(s0)
     36c:	00e78023          	sb	a4,0(a5)
    if(c == '\n' || c == '\r')
     370:	fe744783          	lbu	a5,-25(s0)
     374:	873e                	mv	a4,a5
     376:	47a9                	li	a5,10
     378:	02f70463          	beq	a4,a5,3a0 <gets+0x86>
     37c:	fe744783          	lbu	a5,-25(s0)
     380:	873e                	mv	a4,a5
     382:	47b5                	li	a5,13
     384:	00f70e63          	beq	a4,a5,3a0 <gets+0x86>
  for(i=0; i+1 < max; ){
     388:	fec42783          	lw	a5,-20(s0)
     38c:	2785                	addiw	a5,a5,1
     38e:	0007871b          	sext.w	a4,a5
     392:	fd442783          	lw	a5,-44(s0)
     396:	2781                	sext.w	a5,a5
     398:	f8f74de3          	blt	a4,a5,332 <gets+0x18>
     39c:	a011                	j	3a0 <gets+0x86>
      break;
     39e:	0001                	nop
      break;
  }
  buf[i] = '\0';
     3a0:	fec42783          	lw	a5,-20(s0)
     3a4:	fd843703          	ld	a4,-40(s0)
     3a8:	97ba                	add	a5,a5,a4
     3aa:	00078023          	sb	zero,0(a5)
  return buf;
     3ae:	fd843783          	ld	a5,-40(s0)
}
     3b2:	853e                	mv	a0,a5
     3b4:	70a2                	ld	ra,40(sp)
     3b6:	7402                	ld	s0,32(sp)
     3b8:	6145                	addi	sp,sp,48
     3ba:	8082                	ret

00000000000003bc <stat>:

int
stat(const char *n, struct stat *st)
{
     3bc:	7179                	addi	sp,sp,-48
     3be:	f406                	sd	ra,40(sp)
     3c0:	f022                	sd	s0,32(sp)
     3c2:	1800                	addi	s0,sp,48
     3c4:	fca43c23          	sd	a0,-40(s0)
     3c8:	fcb43823          	sd	a1,-48(s0)
  int fd;
  int r;

  fd = open(n, O_RDONLY);
     3cc:	4581                	li	a1,0
     3ce:	fd843503          	ld	a0,-40(s0)
     3d2:	00000097          	auipc	ra,0x0
     3d6:	288080e7          	jalr	648(ra) # 65a <open>
     3da:	87aa                	mv	a5,a0
     3dc:	fef42623          	sw	a5,-20(s0)
  if(fd < 0)
     3e0:	fec42783          	lw	a5,-20(s0)
     3e4:	2781                	sext.w	a5,a5
     3e6:	0007d463          	bgez	a5,3ee <stat+0x32>
    return -1;
     3ea:	57fd                	li	a5,-1
     3ec:	a035                	j	418 <stat+0x5c>
  r = fstat(fd, st);
     3ee:	fec42783          	lw	a5,-20(s0)
     3f2:	fd043583          	ld	a1,-48(s0)
     3f6:	853e                	mv	a0,a5
     3f8:	00000097          	auipc	ra,0x0
     3fc:	27a080e7          	jalr	634(ra) # 672 <fstat>
     400:	87aa                	mv	a5,a0
     402:	fef42423          	sw	a5,-24(s0)
  close(fd);
     406:	fec42783          	lw	a5,-20(s0)
     40a:	853e                	mv	a0,a5
     40c:	00000097          	auipc	ra,0x0
     410:	236080e7          	jalr	566(ra) # 642 <close>
  return r;
     414:	fe842783          	lw	a5,-24(s0)
}
     418:	853e                	mv	a0,a5
     41a:	70a2                	ld	ra,40(sp)
     41c:	7402                	ld	s0,32(sp)
     41e:	6145                	addi	sp,sp,48
     420:	8082                	ret

0000000000000422 <atoi>:

int
atoi(const char *s)
{
     422:	7179                	addi	sp,sp,-48
     424:	f422                	sd	s0,40(sp)
     426:	1800                	addi	s0,sp,48
     428:	fca43c23          	sd	a0,-40(s0)
  int n;

  n = 0;
     42c:	fe042623          	sw	zero,-20(s0)
  while('0' <= *s && *s <= '9')
     430:	a815                	j	464 <atoi+0x42>
    n = n*10 + *s++ - '0';
     432:	fec42703          	lw	a4,-20(s0)
     436:	87ba                	mv	a5,a4
     438:	0027979b          	slliw	a5,a5,0x2
     43c:	9fb9                	addw	a5,a5,a4
     43e:	0017979b          	slliw	a5,a5,0x1
     442:	0007871b          	sext.w	a4,a5
     446:	fd843783          	ld	a5,-40(s0)
     44a:	00178693          	addi	a3,a5,1
     44e:	fcd43c23          	sd	a3,-40(s0)
     452:	0007c783          	lbu	a5,0(a5)
     456:	2781                	sext.w	a5,a5
     458:	9fb9                	addw	a5,a5,a4
     45a:	2781                	sext.w	a5,a5
     45c:	fd07879b          	addiw	a5,a5,-48
     460:	fef42623          	sw	a5,-20(s0)
  while('0' <= *s && *s <= '9')
     464:	fd843783          	ld	a5,-40(s0)
     468:	0007c783          	lbu	a5,0(a5)
     46c:	873e                	mv	a4,a5
     46e:	02f00793          	li	a5,47
     472:	00e7fb63          	bgeu	a5,a4,488 <atoi+0x66>
     476:	fd843783          	ld	a5,-40(s0)
     47a:	0007c783          	lbu	a5,0(a5)
     47e:	873e                	mv	a4,a5
     480:	03900793          	li	a5,57
     484:	fae7f7e3          	bgeu	a5,a4,432 <atoi+0x10>
  return n;
     488:	fec42783          	lw	a5,-20(s0)
}
     48c:	853e                	mv	a0,a5
     48e:	7422                	ld	s0,40(sp)
     490:	6145                	addi	sp,sp,48
     492:	8082                	ret

0000000000000494 <memmove>:

void*
memmove(void *vdst, const void *vsrc, int n)
{
     494:	7139                	addi	sp,sp,-64
     496:	fc22                	sd	s0,56(sp)
     498:	0080                	addi	s0,sp,64
     49a:	fca43c23          	sd	a0,-40(s0)
     49e:	fcb43823          	sd	a1,-48(s0)
     4a2:	87b2                	mv	a5,a2
     4a4:	fcf42623          	sw	a5,-52(s0)
  char *dst;
  const char *src;

  dst = vdst;
     4a8:	fd843783          	ld	a5,-40(s0)
     4ac:	fef43423          	sd	a5,-24(s0)
  src = vsrc;
     4b0:	fd043783          	ld	a5,-48(s0)
     4b4:	fef43023          	sd	a5,-32(s0)
  if (src > dst) {
     4b8:	fe043703          	ld	a4,-32(s0)
     4bc:	fe843783          	ld	a5,-24(s0)
     4c0:	02e7fc63          	bgeu	a5,a4,4f8 <memmove+0x64>
    while(n-- > 0)
     4c4:	a00d                	j	4e6 <memmove+0x52>
      *dst++ = *src++;
     4c6:	fe043703          	ld	a4,-32(s0)
     4ca:	00170793          	addi	a5,a4,1
     4ce:	fef43023          	sd	a5,-32(s0)
     4d2:	fe843783          	ld	a5,-24(s0)
     4d6:	00178693          	addi	a3,a5,1
     4da:	fed43423          	sd	a3,-24(s0)
     4de:	00074703          	lbu	a4,0(a4)
     4e2:	00e78023          	sb	a4,0(a5)
    while(n-- > 0)
     4e6:	fcc42783          	lw	a5,-52(s0)
     4ea:	fff7871b          	addiw	a4,a5,-1
     4ee:	fce42623          	sw	a4,-52(s0)
     4f2:	fcf04ae3          	bgtz	a5,4c6 <memmove+0x32>
     4f6:	a891                	j	54a <memmove+0xb6>
  } else {
    dst += n;
     4f8:	fcc42783          	lw	a5,-52(s0)
     4fc:	fe843703          	ld	a4,-24(s0)
     500:	97ba                	add	a5,a5,a4
     502:	fef43423          	sd	a5,-24(s0)
    src += n;
     506:	fcc42783          	lw	a5,-52(s0)
     50a:	fe043703          	ld	a4,-32(s0)
     50e:	97ba                	add	a5,a5,a4
     510:	fef43023          	sd	a5,-32(s0)
    while(n-- > 0)
     514:	a01d                	j	53a <memmove+0xa6>
      *--dst = *--src;
     516:	fe043783          	ld	a5,-32(s0)
     51a:	17fd                	addi	a5,a5,-1
     51c:	fef43023          	sd	a5,-32(s0)
     520:	fe843783          	ld	a5,-24(s0)
     524:	17fd                	addi	a5,a5,-1
     526:	fef43423          	sd	a5,-24(s0)
     52a:	fe043783          	ld	a5,-32(s0)
     52e:	0007c703          	lbu	a4,0(a5)
     532:	fe843783          	ld	a5,-24(s0)
     536:	00e78023          	sb	a4,0(a5)
    while(n-- > 0)
     53a:	fcc42783          	lw	a5,-52(s0)
     53e:	fff7871b          	addiw	a4,a5,-1
     542:	fce42623          	sw	a4,-52(s0)
     546:	fcf048e3          	bgtz	a5,516 <memmove+0x82>
  }
  return vdst;
     54a:	fd843783          	ld	a5,-40(s0)
}
     54e:	853e                	mv	a0,a5
     550:	7462                	ld	s0,56(sp)
     552:	6121                	addi	sp,sp,64
     554:	8082                	ret

0000000000000556 <memcmp>:

int
memcmp(const void *s1, const void *s2, uint n)
{
     556:	7139                	addi	sp,sp,-64
     558:	fc22                	sd	s0,56(sp)
     55a:	0080                	addi	s0,sp,64
     55c:	fca43c23          	sd	a0,-40(s0)
     560:	fcb43823          	sd	a1,-48(s0)
     564:	87b2                	mv	a5,a2
     566:	fcf42623          	sw	a5,-52(s0)
  const char *p1 = s1, *p2 = s2;
     56a:	fd843783          	ld	a5,-40(s0)
     56e:	fef43423          	sd	a5,-24(s0)
     572:	fd043783          	ld	a5,-48(s0)
     576:	fef43023          	sd	a5,-32(s0)
  while (n-- > 0) {
     57a:	a0a1                	j	5c2 <memcmp+0x6c>
    if (*p1 != *p2) {
     57c:	fe843783          	ld	a5,-24(s0)
     580:	0007c703          	lbu	a4,0(a5)
     584:	fe043783          	ld	a5,-32(s0)
     588:	0007c783          	lbu	a5,0(a5)
     58c:	02f70163          	beq	a4,a5,5ae <memcmp+0x58>
      return *p1 - *p2;
     590:	fe843783          	ld	a5,-24(s0)
     594:	0007c783          	lbu	a5,0(a5)
     598:	0007871b          	sext.w	a4,a5
     59c:	fe043783          	ld	a5,-32(s0)
     5a0:	0007c783          	lbu	a5,0(a5)
     5a4:	2781                	sext.w	a5,a5
     5a6:	40f707bb          	subw	a5,a4,a5
     5aa:	2781                	sext.w	a5,a5
     5ac:	a01d                	j	5d2 <memcmp+0x7c>
    }
    p1++;
     5ae:	fe843783          	ld	a5,-24(s0)
     5b2:	0785                	addi	a5,a5,1
     5b4:	fef43423          	sd	a5,-24(s0)
    p2++;
     5b8:	fe043783          	ld	a5,-32(s0)
     5bc:	0785                	addi	a5,a5,1
     5be:	fef43023          	sd	a5,-32(s0)
  while (n-- > 0) {
     5c2:	fcc42783          	lw	a5,-52(s0)
     5c6:	fff7871b          	addiw	a4,a5,-1
     5ca:	fce42623          	sw	a4,-52(s0)
     5ce:	f7dd                	bnez	a5,57c <memcmp+0x26>
  }
  return 0;
     5d0:	4781                	li	a5,0
}
     5d2:	853e                	mv	a0,a5
     5d4:	7462                	ld	s0,56(sp)
     5d6:	6121                	addi	sp,sp,64
     5d8:	8082                	ret

00000000000005da <memcpy>:

void *
memcpy(void *dst, const void *src, uint n)
{
     5da:	7179                	addi	sp,sp,-48
     5dc:	f406                	sd	ra,40(sp)
     5de:	f022                	sd	s0,32(sp)
     5e0:	1800                	addi	s0,sp,48
     5e2:	fea43423          	sd	a0,-24(s0)
     5e6:	feb43023          	sd	a1,-32(s0)
     5ea:	87b2                	mv	a5,a2
     5ec:	fcf42e23          	sw	a5,-36(s0)
  return memmove(dst, src, n);
     5f0:	fdc42783          	lw	a5,-36(s0)
     5f4:	863e                	mv	a2,a5
     5f6:	fe043583          	ld	a1,-32(s0)
     5fa:	fe843503          	ld	a0,-24(s0)
     5fe:	00000097          	auipc	ra,0x0
     602:	e96080e7          	jalr	-362(ra) # 494 <memmove>
     606:	87aa                	mv	a5,a0
}
     608:	853e                	mv	a0,a5
     60a:	70a2                	ld	ra,40(sp)
     60c:	7402                	ld	s0,32(sp)
     60e:	6145                	addi	sp,sp,48
     610:	8082                	ret

0000000000000612 <fork>:
# generated by usys.pl - do not edit
#include "kernel/syscall.h"
.global fork
fork:
 li a7, SYS_fork
     612:	4885                	li	a7,1
 ecall
     614:	00000073          	ecall
 ret
     618:	8082                	ret

000000000000061a <exit>:
.global exit
exit:
 li a7, SYS_exit
     61a:	4889                	li	a7,2
 ecall
     61c:	00000073          	ecall
 ret
     620:	8082                	ret

0000000000000622 <wait>:
.global wait
wait:
 li a7, SYS_wait
     622:	488d                	li	a7,3
 ecall
     624:	00000073          	ecall
 ret
     628:	8082                	ret

000000000000062a <pipe>:
.global pipe
pipe:
 li a7, SYS_pipe
     62a:	4891                	li	a7,4
 ecall
     62c:	00000073          	ecall
 ret
     630:	8082                	ret

0000000000000632 <read>:
.global read
read:
 li a7, SYS_read
     632:	4895                	li	a7,5
 ecall
     634:	00000073          	ecall
 ret
     638:	8082                	ret

000000000000063a <write>:
.global write
write:
 li a7, SYS_write
     63a:	48c1                	li	a7,16
 ecall
     63c:	00000073          	ecall
 ret
     640:	8082                	ret

0000000000000642 <close>:
.global close
close:
 li a7, SYS_close
     642:	48d5                	li	a7,21
 ecall
     644:	00000073          	ecall
 ret
     648:	8082                	ret

000000000000064a <kill>:
.global kill
kill:
 li a7, SYS_kill
     64a:	4899                	li	a7,6
 ecall
     64c:	00000073          	ecall
 ret
     650:	8082                	ret

0000000000000652 <exec>:
.global exec
exec:
 li a7, SYS_exec
     652:	489d                	li	a7,7
 ecall
     654:	00000073          	ecall
 ret
     658:	8082                	ret

000000000000065a <open>:
.global open
open:
 li a7, SYS_open
     65a:	48bd                	li	a7,15
 ecall
     65c:	00000073          	ecall
 ret
     660:	8082                	ret

0000000000000662 <mknod>:
.global mknod
mknod:
 li a7, SYS_mknod
     662:	48c5                	li	a7,17
 ecall
     664:	00000073          	ecall
 ret
     668:	8082                	ret

000000000000066a <unlink>:
.global unlink
unlink:
 li a7, SYS_unlink
     66a:	48c9                	li	a7,18
 ecall
     66c:	00000073          	ecall
 ret
     670:	8082                	ret

0000000000000672 <fstat>:
.global fstat
fstat:
 li a7, SYS_fstat
     672:	48a1                	li	a7,8
 ecall
     674:	00000073          	ecall
 ret
     678:	8082                	ret

000000000000067a <link>:
.global link
link:
 li a7, SYS_link
     67a:	48cd                	li	a7,19
 ecall
     67c:	00000073          	ecall
 ret
     680:	8082                	ret

0000000000000682 <mkdir>:
.global mkdir
mkdir:
 li a7, SYS_mkdir
     682:	48d1                	li	a7,20
 ecall
     684:	00000073          	ecall
 ret
     688:	8082                	ret

000000000000068a <chdir>:
.global chdir
chdir:
 li a7, SYS_chdir
     68a:	48a5                	li	a7,9
 ecall
     68c:	00000073          	ecall
 ret
     690:	8082                	ret

0000000000000692 <dup>:
.global dup
dup:
 li a7, SYS_dup
     692:	48a9                	li	a7,10
 ecall
     694:	00000073          	ecall
 ret
     698:	8082                	ret

000000000000069a <getpid>:
.global getpid
getpid:
 li a7, SYS_getpid
     69a:	48ad                	li	a7,11
 ecall
     69c:	00000073          	ecall
 ret
     6a0:	8082                	ret

00000000000006a2 <sbrk>:
.global sbrk
sbrk:
 li a7, SYS_sbrk
     6a2:	48b1                	li	a7,12
 ecall
     6a4:	00000073          	ecall
 ret
     6a8:	8082                	ret

00000000000006aa <sleep>:
.global sleep
sleep:
 li a7, SYS_sleep
     6aa:	48b5                	li	a7,13
 ecall
     6ac:	00000073          	ecall
 ret
     6b0:	8082                	ret

00000000000006b2 <uptime>:
.global uptime
uptime:
 li a7, SYS_uptime
     6b2:	48b9                	li	a7,14
 ecall
     6b4:	00000073          	ecall
 ret
     6b8:	8082                	ret

00000000000006ba <thrdstop>:
.global thrdstop
thrdstop:
 li a7, SYS_thrdstop
     6ba:	48d9                	li	a7,22
 ecall
     6bc:	00000073          	ecall
 ret
     6c0:	8082                	ret

00000000000006c2 <thrdresume>:
.global thrdresume
thrdresume:
 li a7, SYS_thrdresume
     6c2:	48dd                	li	a7,23
 ecall
     6c4:	00000073          	ecall
 ret
     6c8:	8082                	ret

00000000000006ca <cancelthrdstop>:
.global cancelthrdstop
cancelthrdstop:
 li a7, SYS_cancelthrdstop
     6ca:	48e1                	li	a7,24
 ecall
     6cc:	00000073          	ecall
 ret
     6d0:	8082                	ret

00000000000006d2 <putc>:

static char digits[] = "0123456789ABCDEF";

static void
putc(int fd, char c)
{
     6d2:	1101                	addi	sp,sp,-32
     6d4:	ec06                	sd	ra,24(sp)
     6d6:	e822                	sd	s0,16(sp)
     6d8:	1000                	addi	s0,sp,32
     6da:	87aa                	mv	a5,a0
     6dc:	872e                	mv	a4,a1
     6de:	fef42623          	sw	a5,-20(s0)
     6e2:	87ba                	mv	a5,a4
     6e4:	fef405a3          	sb	a5,-21(s0)
  write(fd, &c, 1);
     6e8:	feb40713          	addi	a4,s0,-21
     6ec:	fec42783          	lw	a5,-20(s0)
     6f0:	4605                	li	a2,1
     6f2:	85ba                	mv	a1,a4
     6f4:	853e                	mv	a0,a5
     6f6:	00000097          	auipc	ra,0x0
     6fa:	f44080e7          	jalr	-188(ra) # 63a <write>
}
     6fe:	0001                	nop
     700:	60e2                	ld	ra,24(sp)
     702:	6442                	ld	s0,16(sp)
     704:	6105                	addi	sp,sp,32
     706:	8082                	ret

0000000000000708 <printint>:

static void
printint(int fd, int xx, int base, int sgn)
{
     708:	7139                	addi	sp,sp,-64
     70a:	fc06                	sd	ra,56(sp)
     70c:	f822                	sd	s0,48(sp)
     70e:	0080                	addi	s0,sp,64
     710:	87aa                	mv	a5,a0
     712:	8736                	mv	a4,a3
     714:	fcf42623          	sw	a5,-52(s0)
     718:	87ae                	mv	a5,a1
     71a:	fcf42423          	sw	a5,-56(s0)
     71e:	87b2                	mv	a5,a2
     720:	fcf42223          	sw	a5,-60(s0)
     724:	87ba                	mv	a5,a4
     726:	fcf42023          	sw	a5,-64(s0)
  char buf[16];
  int i, neg;
  uint x;

  neg = 0;
     72a:	fe042423          	sw	zero,-24(s0)
  if(sgn && xx < 0){
     72e:	fc042783          	lw	a5,-64(s0)
     732:	2781                	sext.w	a5,a5
     734:	c38d                	beqz	a5,756 <printint+0x4e>
     736:	fc842783          	lw	a5,-56(s0)
     73a:	2781                	sext.w	a5,a5
     73c:	0007dd63          	bgez	a5,756 <printint+0x4e>
    neg = 1;
     740:	4785                	li	a5,1
     742:	fef42423          	sw	a5,-24(s0)
    x = -xx;
     746:	fc842783          	lw	a5,-56(s0)
     74a:	40f007bb          	negw	a5,a5
     74e:	2781                	sext.w	a5,a5
     750:	fef42223          	sw	a5,-28(s0)
     754:	a029                	j	75e <printint+0x56>
  } else {
    x = xx;
     756:	fc842783          	lw	a5,-56(s0)
     75a:	fef42223          	sw	a5,-28(s0)
  }

  i = 0;
     75e:	fe042623          	sw	zero,-20(s0)
  do{
    buf[i++] = digits[x % base];
     762:	fc442783          	lw	a5,-60(s0)
     766:	fe442703          	lw	a4,-28(s0)
     76a:	02f777bb          	remuw	a5,a4,a5
     76e:	0007861b          	sext.w	a2,a5
     772:	fec42783          	lw	a5,-20(s0)
     776:	0017871b          	addiw	a4,a5,1
     77a:	fee42623          	sw	a4,-20(s0)
     77e:	00002697          	auipc	a3,0x2
     782:	c6a68693          	addi	a3,a3,-918 # 23e8 <digits>
     786:	02061713          	slli	a4,a2,0x20
     78a:	9301                	srli	a4,a4,0x20
     78c:	9736                	add	a4,a4,a3
     78e:	00074703          	lbu	a4,0(a4)
     792:	ff040693          	addi	a3,s0,-16
     796:	97b6                	add	a5,a5,a3
     798:	fee78023          	sb	a4,-32(a5)
  }while((x /= base) != 0);
     79c:	fc442783          	lw	a5,-60(s0)
     7a0:	fe442703          	lw	a4,-28(s0)
     7a4:	02f757bb          	divuw	a5,a4,a5
     7a8:	fef42223          	sw	a5,-28(s0)
     7ac:	fe442783          	lw	a5,-28(s0)
     7b0:	2781                	sext.w	a5,a5
     7b2:	fbc5                	bnez	a5,762 <printint+0x5a>
  if(neg)
     7b4:	fe842783          	lw	a5,-24(s0)
     7b8:	2781                	sext.w	a5,a5
     7ba:	cf95                	beqz	a5,7f6 <printint+0xee>
    buf[i++] = '-';
     7bc:	fec42783          	lw	a5,-20(s0)
     7c0:	0017871b          	addiw	a4,a5,1
     7c4:	fee42623          	sw	a4,-20(s0)
     7c8:	ff040713          	addi	a4,s0,-16
     7cc:	97ba                	add	a5,a5,a4
     7ce:	02d00713          	li	a4,45
     7d2:	fee78023          	sb	a4,-32(a5)

  while(--i >= 0)
     7d6:	a005                	j	7f6 <printint+0xee>
    putc(fd, buf[i]);
     7d8:	fec42783          	lw	a5,-20(s0)
     7dc:	ff040713          	addi	a4,s0,-16
     7e0:	97ba                	add	a5,a5,a4
     7e2:	fe07c703          	lbu	a4,-32(a5)
     7e6:	fcc42783          	lw	a5,-52(s0)
     7ea:	85ba                	mv	a1,a4
     7ec:	853e                	mv	a0,a5
     7ee:	00000097          	auipc	ra,0x0
     7f2:	ee4080e7          	jalr	-284(ra) # 6d2 <putc>
  while(--i >= 0)
     7f6:	fec42783          	lw	a5,-20(s0)
     7fa:	37fd                	addiw	a5,a5,-1
     7fc:	fef42623          	sw	a5,-20(s0)
     800:	fec42783          	lw	a5,-20(s0)
     804:	2781                	sext.w	a5,a5
     806:	fc07d9e3          	bgez	a5,7d8 <printint+0xd0>
}
     80a:	0001                	nop
     80c:	0001                	nop
     80e:	70e2                	ld	ra,56(sp)
     810:	7442                	ld	s0,48(sp)
     812:	6121                	addi	sp,sp,64
     814:	8082                	ret

0000000000000816 <printptr>:

static void
printptr(int fd, uint64 x) {
     816:	7179                	addi	sp,sp,-48
     818:	f406                	sd	ra,40(sp)
     81a:	f022                	sd	s0,32(sp)
     81c:	1800                	addi	s0,sp,48
     81e:	87aa                	mv	a5,a0
     820:	fcb43823          	sd	a1,-48(s0)
     824:	fcf42e23          	sw	a5,-36(s0)
  int i;
  putc(fd, '0');
     828:	fdc42783          	lw	a5,-36(s0)
     82c:	03000593          	li	a1,48
     830:	853e                	mv	a0,a5
     832:	00000097          	auipc	ra,0x0
     836:	ea0080e7          	jalr	-352(ra) # 6d2 <putc>
  putc(fd, 'x');
     83a:	fdc42783          	lw	a5,-36(s0)
     83e:	07800593          	li	a1,120
     842:	853e                	mv	a0,a5
     844:	00000097          	auipc	ra,0x0
     848:	e8e080e7          	jalr	-370(ra) # 6d2 <putc>
  for (i = 0; i < (sizeof(uint64) * 2); i++, x <<= 4)
     84c:	fe042623          	sw	zero,-20(s0)
     850:	a82d                	j	88a <printptr+0x74>
    putc(fd, digits[x >> (sizeof(uint64) * 8 - 4)]);
     852:	fd043783          	ld	a5,-48(s0)
     856:	93f1                	srli	a5,a5,0x3c
     858:	00002717          	auipc	a4,0x2
     85c:	b9070713          	addi	a4,a4,-1136 # 23e8 <digits>
     860:	97ba                	add	a5,a5,a4
     862:	0007c703          	lbu	a4,0(a5)
     866:	fdc42783          	lw	a5,-36(s0)
     86a:	85ba                	mv	a1,a4
     86c:	853e                	mv	a0,a5
     86e:	00000097          	auipc	ra,0x0
     872:	e64080e7          	jalr	-412(ra) # 6d2 <putc>
  for (i = 0; i < (sizeof(uint64) * 2); i++, x <<= 4)
     876:	fec42783          	lw	a5,-20(s0)
     87a:	2785                	addiw	a5,a5,1
     87c:	fef42623          	sw	a5,-20(s0)
     880:	fd043783          	ld	a5,-48(s0)
     884:	0792                	slli	a5,a5,0x4
     886:	fcf43823          	sd	a5,-48(s0)
     88a:	fec42783          	lw	a5,-20(s0)
     88e:	873e                	mv	a4,a5
     890:	47bd                	li	a5,15
     892:	fce7f0e3          	bgeu	a5,a4,852 <printptr+0x3c>
}
     896:	0001                	nop
     898:	0001                	nop
     89a:	70a2                	ld	ra,40(sp)
     89c:	7402                	ld	s0,32(sp)
     89e:	6145                	addi	sp,sp,48
     8a0:	8082                	ret

00000000000008a2 <vprintf>:

// Print to the given fd. Only understands %d, %x, %p, %s.
void
vprintf(int fd, const char *fmt, va_list ap)
{
     8a2:	715d                	addi	sp,sp,-80
     8a4:	e486                	sd	ra,72(sp)
     8a6:	e0a2                	sd	s0,64(sp)
     8a8:	0880                	addi	s0,sp,80
     8aa:	87aa                	mv	a5,a0
     8ac:	fcb43023          	sd	a1,-64(s0)
     8b0:	fac43c23          	sd	a2,-72(s0)
     8b4:	fcf42623          	sw	a5,-52(s0)
  char *s;
  int c, i, state;

  state = 0;
     8b8:	fe042023          	sw	zero,-32(s0)
  for(i = 0; fmt[i]; i++){
     8bc:	fe042223          	sw	zero,-28(s0)
     8c0:	a42d                	j	aea <vprintf+0x248>
    c = fmt[i] & 0xff;
     8c2:	fe442783          	lw	a5,-28(s0)
     8c6:	fc043703          	ld	a4,-64(s0)
     8ca:	97ba                	add	a5,a5,a4
     8cc:	0007c783          	lbu	a5,0(a5)
     8d0:	fcf42e23          	sw	a5,-36(s0)
    if(state == 0){
     8d4:	fe042783          	lw	a5,-32(s0)
     8d8:	2781                	sext.w	a5,a5
     8da:	eb9d                	bnez	a5,910 <vprintf+0x6e>
      if(c == '%'){
     8dc:	fdc42783          	lw	a5,-36(s0)
     8e0:	0007871b          	sext.w	a4,a5
     8e4:	02500793          	li	a5,37
     8e8:	00f71763          	bne	a4,a5,8f6 <vprintf+0x54>
        state = '%';
     8ec:	02500793          	li	a5,37
     8f0:	fef42023          	sw	a5,-32(s0)
     8f4:	a2f5                	j	ae0 <vprintf+0x23e>
      } else {
        putc(fd, c);
     8f6:	fdc42783          	lw	a5,-36(s0)
     8fa:	0ff7f713          	andi	a4,a5,255
     8fe:	fcc42783          	lw	a5,-52(s0)
     902:	85ba                	mv	a1,a4
     904:	853e                	mv	a0,a5
     906:	00000097          	auipc	ra,0x0
     90a:	dcc080e7          	jalr	-564(ra) # 6d2 <putc>
     90e:	aac9                	j	ae0 <vprintf+0x23e>
      }
    } else if(state == '%'){
     910:	fe042783          	lw	a5,-32(s0)
     914:	0007871b          	sext.w	a4,a5
     918:	02500793          	li	a5,37
     91c:	1cf71263          	bne	a4,a5,ae0 <vprintf+0x23e>
      if(c == 'd'){
     920:	fdc42783          	lw	a5,-36(s0)
     924:	0007871b          	sext.w	a4,a5
     928:	06400793          	li	a5,100
     92c:	02f71463          	bne	a4,a5,954 <vprintf+0xb2>
        printint(fd, va_arg(ap, int), 10, 1);
     930:	fb843783          	ld	a5,-72(s0)
     934:	00878713          	addi	a4,a5,8
     938:	fae43c23          	sd	a4,-72(s0)
     93c:	4398                	lw	a4,0(a5)
     93e:	fcc42783          	lw	a5,-52(s0)
     942:	4685                	li	a3,1
     944:	4629                	li	a2,10
     946:	85ba                	mv	a1,a4
     948:	853e                	mv	a0,a5
     94a:	00000097          	auipc	ra,0x0
     94e:	dbe080e7          	jalr	-578(ra) # 708 <printint>
     952:	a269                	j	adc <vprintf+0x23a>
      } else if(c == 'l') {
     954:	fdc42783          	lw	a5,-36(s0)
     958:	0007871b          	sext.w	a4,a5
     95c:	06c00793          	li	a5,108
     960:	02f71663          	bne	a4,a5,98c <vprintf+0xea>
        printint(fd, va_arg(ap, uint64), 10, 0);
     964:	fb843783          	ld	a5,-72(s0)
     968:	00878713          	addi	a4,a5,8
     96c:	fae43c23          	sd	a4,-72(s0)
     970:	639c                	ld	a5,0(a5)
     972:	0007871b          	sext.w	a4,a5
     976:	fcc42783          	lw	a5,-52(s0)
     97a:	4681                	li	a3,0
     97c:	4629                	li	a2,10
     97e:	85ba                	mv	a1,a4
     980:	853e                	mv	a0,a5
     982:	00000097          	auipc	ra,0x0
     986:	d86080e7          	jalr	-634(ra) # 708 <printint>
     98a:	aa89                	j	adc <vprintf+0x23a>
      } else if(c == 'x') {
     98c:	fdc42783          	lw	a5,-36(s0)
     990:	0007871b          	sext.w	a4,a5
     994:	07800793          	li	a5,120
     998:	02f71463          	bne	a4,a5,9c0 <vprintf+0x11e>
        printint(fd, va_arg(ap, int), 16, 0);
     99c:	fb843783          	ld	a5,-72(s0)
     9a0:	00878713          	addi	a4,a5,8
     9a4:	fae43c23          	sd	a4,-72(s0)
     9a8:	4398                	lw	a4,0(a5)
     9aa:	fcc42783          	lw	a5,-52(s0)
     9ae:	4681                	li	a3,0
     9b0:	4641                	li	a2,16
     9b2:	85ba                	mv	a1,a4
     9b4:	853e                	mv	a0,a5
     9b6:	00000097          	auipc	ra,0x0
     9ba:	d52080e7          	jalr	-686(ra) # 708 <printint>
     9be:	aa39                	j	adc <vprintf+0x23a>
      } else if(c == 'p') {
     9c0:	fdc42783          	lw	a5,-36(s0)
     9c4:	0007871b          	sext.w	a4,a5
     9c8:	07000793          	li	a5,112
     9cc:	02f71263          	bne	a4,a5,9f0 <vprintf+0x14e>
        printptr(fd, va_arg(ap, uint64));
     9d0:	fb843783          	ld	a5,-72(s0)
     9d4:	00878713          	addi	a4,a5,8
     9d8:	fae43c23          	sd	a4,-72(s0)
     9dc:	6398                	ld	a4,0(a5)
     9de:	fcc42783          	lw	a5,-52(s0)
     9e2:	85ba                	mv	a1,a4
     9e4:	853e                	mv	a0,a5
     9e6:	00000097          	auipc	ra,0x0
     9ea:	e30080e7          	jalr	-464(ra) # 816 <printptr>
     9ee:	a0fd                	j	adc <vprintf+0x23a>
      } else if(c == 's'){
     9f0:	fdc42783          	lw	a5,-36(s0)
     9f4:	0007871b          	sext.w	a4,a5
     9f8:	07300793          	li	a5,115
     9fc:	04f71c63          	bne	a4,a5,a54 <vprintf+0x1b2>
        s = va_arg(ap, char*);
     a00:	fb843783          	ld	a5,-72(s0)
     a04:	00878713          	addi	a4,a5,8
     a08:	fae43c23          	sd	a4,-72(s0)
     a0c:	639c                	ld	a5,0(a5)
     a0e:	fef43423          	sd	a5,-24(s0)
        if(s == 0)
     a12:	fe843783          	ld	a5,-24(s0)
     a16:	eb8d                	bnez	a5,a48 <vprintf+0x1a6>
          s = "(null)";
     a18:	00001797          	auipc	a5,0x1
     a1c:	75078793          	addi	a5,a5,1872 # 2168 <schedule_edf_cbs+0x512>
     a20:	fef43423          	sd	a5,-24(s0)
        while(*s != 0){
     a24:	a015                	j	a48 <vprintf+0x1a6>
          putc(fd, *s);
     a26:	fe843783          	ld	a5,-24(s0)
     a2a:	0007c703          	lbu	a4,0(a5)
     a2e:	fcc42783          	lw	a5,-52(s0)
     a32:	85ba                	mv	a1,a4
     a34:	853e                	mv	a0,a5
     a36:	00000097          	auipc	ra,0x0
     a3a:	c9c080e7          	jalr	-868(ra) # 6d2 <putc>
          s++;
     a3e:	fe843783          	ld	a5,-24(s0)
     a42:	0785                	addi	a5,a5,1
     a44:	fef43423          	sd	a5,-24(s0)
        while(*s != 0){
     a48:	fe843783          	ld	a5,-24(s0)
     a4c:	0007c783          	lbu	a5,0(a5)
     a50:	fbf9                	bnez	a5,a26 <vprintf+0x184>
     a52:	a069                	j	adc <vprintf+0x23a>
        }
      } else if(c == 'c'){
     a54:	fdc42783          	lw	a5,-36(s0)
     a58:	0007871b          	sext.w	a4,a5
     a5c:	06300793          	li	a5,99
     a60:	02f71463          	bne	a4,a5,a88 <vprintf+0x1e6>
        putc(fd, va_arg(ap, uint));
     a64:	fb843783          	ld	a5,-72(s0)
     a68:	00878713          	addi	a4,a5,8
     a6c:	fae43c23          	sd	a4,-72(s0)
     a70:	439c                	lw	a5,0(a5)
     a72:	0ff7f713          	andi	a4,a5,255
     a76:	fcc42783          	lw	a5,-52(s0)
     a7a:	85ba                	mv	a1,a4
     a7c:	853e                	mv	a0,a5
     a7e:	00000097          	auipc	ra,0x0
     a82:	c54080e7          	jalr	-940(ra) # 6d2 <putc>
     a86:	a899                	j	adc <vprintf+0x23a>
      } else if(c == '%'){
     a88:	fdc42783          	lw	a5,-36(s0)
     a8c:	0007871b          	sext.w	a4,a5
     a90:	02500793          	li	a5,37
     a94:	00f71f63          	bne	a4,a5,ab2 <vprintf+0x210>
        putc(fd, c);
     a98:	fdc42783          	lw	a5,-36(s0)
     a9c:	0ff7f713          	andi	a4,a5,255
     aa0:	fcc42783          	lw	a5,-52(s0)
     aa4:	85ba                	mv	a1,a4
     aa6:	853e                	mv	a0,a5
     aa8:	00000097          	auipc	ra,0x0
     aac:	c2a080e7          	jalr	-982(ra) # 6d2 <putc>
     ab0:	a035                	j	adc <vprintf+0x23a>
      } else {
        // Unknown % sequence.  Print it to draw attention.
        putc(fd, '%');
     ab2:	fcc42783          	lw	a5,-52(s0)
     ab6:	02500593          	li	a1,37
     aba:	853e                	mv	a0,a5
     abc:	00000097          	auipc	ra,0x0
     ac0:	c16080e7          	jalr	-1002(ra) # 6d2 <putc>
        putc(fd, c);
     ac4:	fdc42783          	lw	a5,-36(s0)
     ac8:	0ff7f713          	andi	a4,a5,255
     acc:	fcc42783          	lw	a5,-52(s0)
     ad0:	85ba                	mv	a1,a4
     ad2:	853e                	mv	a0,a5
     ad4:	00000097          	auipc	ra,0x0
     ad8:	bfe080e7          	jalr	-1026(ra) # 6d2 <putc>
      }
      state = 0;
     adc:	fe042023          	sw	zero,-32(s0)
  for(i = 0; fmt[i]; i++){
     ae0:	fe442783          	lw	a5,-28(s0)
     ae4:	2785                	addiw	a5,a5,1
     ae6:	fef42223          	sw	a5,-28(s0)
     aea:	fe442783          	lw	a5,-28(s0)
     aee:	fc043703          	ld	a4,-64(s0)
     af2:	97ba                	add	a5,a5,a4
     af4:	0007c783          	lbu	a5,0(a5)
     af8:	dc0795e3          	bnez	a5,8c2 <vprintf+0x20>
    }
  }
}
     afc:	0001                	nop
     afe:	0001                	nop
     b00:	60a6                	ld	ra,72(sp)
     b02:	6406                	ld	s0,64(sp)
     b04:	6161                	addi	sp,sp,80
     b06:	8082                	ret

0000000000000b08 <fprintf>:

void
fprintf(int fd, const char *fmt, ...)
{
     b08:	7159                	addi	sp,sp,-112
     b0a:	fc06                	sd	ra,56(sp)
     b0c:	f822                	sd	s0,48(sp)
     b0e:	0080                	addi	s0,sp,64
     b10:	fcb43823          	sd	a1,-48(s0)
     b14:	e010                	sd	a2,0(s0)
     b16:	e414                	sd	a3,8(s0)
     b18:	e818                	sd	a4,16(s0)
     b1a:	ec1c                	sd	a5,24(s0)
     b1c:	03043023          	sd	a6,32(s0)
     b20:	03143423          	sd	a7,40(s0)
     b24:	87aa                	mv	a5,a0
     b26:	fcf42e23          	sw	a5,-36(s0)
  va_list ap;

  va_start(ap, fmt);
     b2a:	03040793          	addi	a5,s0,48
     b2e:	fcf43423          	sd	a5,-56(s0)
     b32:	fc843783          	ld	a5,-56(s0)
     b36:	fd078793          	addi	a5,a5,-48
     b3a:	fef43423          	sd	a5,-24(s0)
  vprintf(fd, fmt, ap);
     b3e:	fe843703          	ld	a4,-24(s0)
     b42:	fdc42783          	lw	a5,-36(s0)
     b46:	863a                	mv	a2,a4
     b48:	fd043583          	ld	a1,-48(s0)
     b4c:	853e                	mv	a0,a5
     b4e:	00000097          	auipc	ra,0x0
     b52:	d54080e7          	jalr	-684(ra) # 8a2 <vprintf>
}
     b56:	0001                	nop
     b58:	70e2                	ld	ra,56(sp)
     b5a:	7442                	ld	s0,48(sp)
     b5c:	6165                	addi	sp,sp,112
     b5e:	8082                	ret

0000000000000b60 <printf>:

void
printf(const char *fmt, ...)
{
     b60:	7159                	addi	sp,sp,-112
     b62:	f406                	sd	ra,40(sp)
     b64:	f022                	sd	s0,32(sp)
     b66:	1800                	addi	s0,sp,48
     b68:	fca43c23          	sd	a0,-40(s0)
     b6c:	e40c                	sd	a1,8(s0)
     b6e:	e810                	sd	a2,16(s0)
     b70:	ec14                	sd	a3,24(s0)
     b72:	f018                	sd	a4,32(s0)
     b74:	f41c                	sd	a5,40(s0)
     b76:	03043823          	sd	a6,48(s0)
     b7a:	03143c23          	sd	a7,56(s0)
  va_list ap;

  va_start(ap, fmt);
     b7e:	04040793          	addi	a5,s0,64
     b82:	fcf43823          	sd	a5,-48(s0)
     b86:	fd043783          	ld	a5,-48(s0)
     b8a:	fc878793          	addi	a5,a5,-56
     b8e:	fef43423          	sd	a5,-24(s0)
  vprintf(1, fmt, ap);
     b92:	fe843783          	ld	a5,-24(s0)
     b96:	863e                	mv	a2,a5
     b98:	fd843583          	ld	a1,-40(s0)
     b9c:	4505                	li	a0,1
     b9e:	00000097          	auipc	ra,0x0
     ba2:	d04080e7          	jalr	-764(ra) # 8a2 <vprintf>
}
     ba6:	0001                	nop
     ba8:	70a2                	ld	ra,40(sp)
     baa:	7402                	ld	s0,32(sp)
     bac:	6165                	addi	sp,sp,112
     bae:	8082                	ret

0000000000000bb0 <free>:
static Header base;
static Header *freep;

void
free(void *ap)
{
     bb0:	7179                	addi	sp,sp,-48
     bb2:	f422                	sd	s0,40(sp)
     bb4:	1800                	addi	s0,sp,48
     bb6:	fca43c23          	sd	a0,-40(s0)
  Header *bp, *p;

  bp = (Header*)ap - 1;
     bba:	fd843783          	ld	a5,-40(s0)
     bbe:	17c1                	addi	a5,a5,-16
     bc0:	fef43023          	sd	a5,-32(s0)
  for(p = freep; !(bp > p && bp < p->s.ptr); p = p->s.ptr)
     bc4:	00002797          	auipc	a5,0x2
     bc8:	87c78793          	addi	a5,a5,-1924 # 2440 <freep>
     bcc:	639c                	ld	a5,0(a5)
     bce:	fef43423          	sd	a5,-24(s0)
     bd2:	a815                	j	c06 <free+0x56>
    if(p >= p->s.ptr && (bp > p || bp < p->s.ptr))
     bd4:	fe843783          	ld	a5,-24(s0)
     bd8:	639c                	ld	a5,0(a5)
     bda:	fe843703          	ld	a4,-24(s0)
     bde:	00f76f63          	bltu	a4,a5,bfc <free+0x4c>
     be2:	fe043703          	ld	a4,-32(s0)
     be6:	fe843783          	ld	a5,-24(s0)
     bea:	02e7eb63          	bltu	a5,a4,c20 <free+0x70>
     bee:	fe843783          	ld	a5,-24(s0)
     bf2:	639c                	ld	a5,0(a5)
     bf4:	fe043703          	ld	a4,-32(s0)
     bf8:	02f76463          	bltu	a4,a5,c20 <free+0x70>
  for(p = freep; !(bp > p && bp < p->s.ptr); p = p->s.ptr)
     bfc:	fe843783          	ld	a5,-24(s0)
     c00:	639c                	ld	a5,0(a5)
     c02:	fef43423          	sd	a5,-24(s0)
     c06:	fe043703          	ld	a4,-32(s0)
     c0a:	fe843783          	ld	a5,-24(s0)
     c0e:	fce7f3e3          	bgeu	a5,a4,bd4 <free+0x24>
     c12:	fe843783          	ld	a5,-24(s0)
     c16:	639c                	ld	a5,0(a5)
     c18:	fe043703          	ld	a4,-32(s0)
     c1c:	faf77ce3          	bgeu	a4,a5,bd4 <free+0x24>
      break;
  if(bp + bp->s.size == p->s.ptr){
     c20:	fe043783          	ld	a5,-32(s0)
     c24:	479c                	lw	a5,8(a5)
     c26:	1782                	slli	a5,a5,0x20
     c28:	9381                	srli	a5,a5,0x20
     c2a:	0792                	slli	a5,a5,0x4
     c2c:	fe043703          	ld	a4,-32(s0)
     c30:	973e                	add	a4,a4,a5
     c32:	fe843783          	ld	a5,-24(s0)
     c36:	639c                	ld	a5,0(a5)
     c38:	02f71763          	bne	a4,a5,c66 <free+0xb6>
    bp->s.size += p->s.ptr->s.size;
     c3c:	fe043783          	ld	a5,-32(s0)
     c40:	4798                	lw	a4,8(a5)
     c42:	fe843783          	ld	a5,-24(s0)
     c46:	639c                	ld	a5,0(a5)
     c48:	479c                	lw	a5,8(a5)
     c4a:	9fb9                	addw	a5,a5,a4
     c4c:	0007871b          	sext.w	a4,a5
     c50:	fe043783          	ld	a5,-32(s0)
     c54:	c798                	sw	a4,8(a5)
    bp->s.ptr = p->s.ptr->s.ptr;
     c56:	fe843783          	ld	a5,-24(s0)
     c5a:	639c                	ld	a5,0(a5)
     c5c:	6398                	ld	a4,0(a5)
     c5e:	fe043783          	ld	a5,-32(s0)
     c62:	e398                	sd	a4,0(a5)
     c64:	a039                	j	c72 <free+0xc2>
  } else
    bp->s.ptr = p->s.ptr;
     c66:	fe843783          	ld	a5,-24(s0)
     c6a:	6398                	ld	a4,0(a5)
     c6c:	fe043783          	ld	a5,-32(s0)
     c70:	e398                	sd	a4,0(a5)
  if(p + p->s.size == bp){
     c72:	fe843783          	ld	a5,-24(s0)
     c76:	479c                	lw	a5,8(a5)
     c78:	1782                	slli	a5,a5,0x20
     c7a:	9381                	srli	a5,a5,0x20
     c7c:	0792                	slli	a5,a5,0x4
     c7e:	fe843703          	ld	a4,-24(s0)
     c82:	97ba                	add	a5,a5,a4
     c84:	fe043703          	ld	a4,-32(s0)
     c88:	02f71563          	bne	a4,a5,cb2 <free+0x102>
    p->s.size += bp->s.size;
     c8c:	fe843783          	ld	a5,-24(s0)
     c90:	4798                	lw	a4,8(a5)
     c92:	fe043783          	ld	a5,-32(s0)
     c96:	479c                	lw	a5,8(a5)
     c98:	9fb9                	addw	a5,a5,a4
     c9a:	0007871b          	sext.w	a4,a5
     c9e:	fe843783          	ld	a5,-24(s0)
     ca2:	c798                	sw	a4,8(a5)
    p->s.ptr = bp->s.ptr;
     ca4:	fe043783          	ld	a5,-32(s0)
     ca8:	6398                	ld	a4,0(a5)
     caa:	fe843783          	ld	a5,-24(s0)
     cae:	e398                	sd	a4,0(a5)
     cb0:	a031                	j	cbc <free+0x10c>
  } else
    p->s.ptr = bp;
     cb2:	fe843783          	ld	a5,-24(s0)
     cb6:	fe043703          	ld	a4,-32(s0)
     cba:	e398                	sd	a4,0(a5)
  freep = p;
     cbc:	00001797          	auipc	a5,0x1
     cc0:	78478793          	addi	a5,a5,1924 # 2440 <freep>
     cc4:	fe843703          	ld	a4,-24(s0)
     cc8:	e398                	sd	a4,0(a5)
}
     cca:	0001                	nop
     ccc:	7422                	ld	s0,40(sp)
     cce:	6145                	addi	sp,sp,48
     cd0:	8082                	ret

0000000000000cd2 <morecore>:

static Header*
morecore(uint nu)
{
     cd2:	7179                	addi	sp,sp,-48
     cd4:	f406                	sd	ra,40(sp)
     cd6:	f022                	sd	s0,32(sp)
     cd8:	1800                	addi	s0,sp,48
     cda:	87aa                	mv	a5,a0
     cdc:	fcf42e23          	sw	a5,-36(s0)
  char *p;
  Header *hp;

  if(nu < 4096)
     ce0:	fdc42783          	lw	a5,-36(s0)
     ce4:	0007871b          	sext.w	a4,a5
     ce8:	6785                	lui	a5,0x1
     cea:	00f77563          	bgeu	a4,a5,cf4 <morecore+0x22>
    nu = 4096;
     cee:	6785                	lui	a5,0x1
     cf0:	fcf42e23          	sw	a5,-36(s0)
  p = sbrk(nu * sizeof(Header));
     cf4:	fdc42783          	lw	a5,-36(s0)
     cf8:	0047979b          	slliw	a5,a5,0x4
     cfc:	2781                	sext.w	a5,a5
     cfe:	2781                	sext.w	a5,a5
     d00:	853e                	mv	a0,a5
     d02:	00000097          	auipc	ra,0x0
     d06:	9a0080e7          	jalr	-1632(ra) # 6a2 <sbrk>
     d0a:	fea43423          	sd	a0,-24(s0)
  if(p == (char*)-1)
     d0e:	fe843703          	ld	a4,-24(s0)
     d12:	57fd                	li	a5,-1
     d14:	00f71463          	bne	a4,a5,d1c <morecore+0x4a>
    return 0;
     d18:	4781                	li	a5,0
     d1a:	a03d                	j	d48 <morecore+0x76>
  hp = (Header*)p;
     d1c:	fe843783          	ld	a5,-24(s0)
     d20:	fef43023          	sd	a5,-32(s0)
  hp->s.size = nu;
     d24:	fe043783          	ld	a5,-32(s0)
     d28:	fdc42703          	lw	a4,-36(s0)
     d2c:	c798                	sw	a4,8(a5)
  free((void*)(hp + 1));
     d2e:	fe043783          	ld	a5,-32(s0)
     d32:	07c1                	addi	a5,a5,16
     d34:	853e                	mv	a0,a5
     d36:	00000097          	auipc	ra,0x0
     d3a:	e7a080e7          	jalr	-390(ra) # bb0 <free>
  return freep;
     d3e:	00001797          	auipc	a5,0x1
     d42:	70278793          	addi	a5,a5,1794 # 2440 <freep>
     d46:	639c                	ld	a5,0(a5)
}
     d48:	853e                	mv	a0,a5
     d4a:	70a2                	ld	ra,40(sp)
     d4c:	7402                	ld	s0,32(sp)
     d4e:	6145                	addi	sp,sp,48
     d50:	8082                	ret

0000000000000d52 <malloc>:

void*
malloc(uint nbytes)
{
     d52:	7139                	addi	sp,sp,-64
     d54:	fc06                	sd	ra,56(sp)
     d56:	f822                	sd	s0,48(sp)
     d58:	0080                	addi	s0,sp,64
     d5a:	87aa                	mv	a5,a0
     d5c:	fcf42623          	sw	a5,-52(s0)
  Header *p, *prevp;
  uint nunits;

  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
     d60:	fcc46783          	lwu	a5,-52(s0)
     d64:	07bd                	addi	a5,a5,15
     d66:	8391                	srli	a5,a5,0x4
     d68:	2781                	sext.w	a5,a5
     d6a:	2785                	addiw	a5,a5,1
     d6c:	fcf42e23          	sw	a5,-36(s0)
  if((prevp = freep) == 0){
     d70:	00001797          	auipc	a5,0x1
     d74:	6d078793          	addi	a5,a5,1744 # 2440 <freep>
     d78:	639c                	ld	a5,0(a5)
     d7a:	fef43023          	sd	a5,-32(s0)
     d7e:	fe043783          	ld	a5,-32(s0)
     d82:	ef95                	bnez	a5,dbe <malloc+0x6c>
    base.s.ptr = freep = prevp = &base;
     d84:	00001797          	auipc	a5,0x1
     d88:	6ac78793          	addi	a5,a5,1708 # 2430 <base>
     d8c:	fef43023          	sd	a5,-32(s0)
     d90:	00001797          	auipc	a5,0x1
     d94:	6b078793          	addi	a5,a5,1712 # 2440 <freep>
     d98:	fe043703          	ld	a4,-32(s0)
     d9c:	e398                	sd	a4,0(a5)
     d9e:	00001797          	auipc	a5,0x1
     da2:	6a278793          	addi	a5,a5,1698 # 2440 <freep>
     da6:	6398                	ld	a4,0(a5)
     da8:	00001797          	auipc	a5,0x1
     dac:	68878793          	addi	a5,a5,1672 # 2430 <base>
     db0:	e398                	sd	a4,0(a5)
    base.s.size = 0;
     db2:	00001797          	auipc	a5,0x1
     db6:	67e78793          	addi	a5,a5,1662 # 2430 <base>
     dba:	0007a423          	sw	zero,8(a5)
  }
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
     dbe:	fe043783          	ld	a5,-32(s0)
     dc2:	639c                	ld	a5,0(a5)
     dc4:	fef43423          	sd	a5,-24(s0)
    if(p->s.size >= nunits){
     dc8:	fe843783          	ld	a5,-24(s0)
     dcc:	4798                	lw	a4,8(a5)
     dce:	fdc42783          	lw	a5,-36(s0)
     dd2:	2781                	sext.w	a5,a5
     dd4:	06f76863          	bltu	a4,a5,e44 <malloc+0xf2>
      if(p->s.size == nunits)
     dd8:	fe843783          	ld	a5,-24(s0)
     ddc:	4798                	lw	a4,8(a5)
     dde:	fdc42783          	lw	a5,-36(s0)
     de2:	2781                	sext.w	a5,a5
     de4:	00e79963          	bne	a5,a4,df6 <malloc+0xa4>
        prevp->s.ptr = p->s.ptr;
     de8:	fe843783          	ld	a5,-24(s0)
     dec:	6398                	ld	a4,0(a5)
     dee:	fe043783          	ld	a5,-32(s0)
     df2:	e398                	sd	a4,0(a5)
     df4:	a82d                	j	e2e <malloc+0xdc>
      else {
        p->s.size -= nunits;
     df6:	fe843783          	ld	a5,-24(s0)
     dfa:	4798                	lw	a4,8(a5)
     dfc:	fdc42783          	lw	a5,-36(s0)
     e00:	40f707bb          	subw	a5,a4,a5
     e04:	0007871b          	sext.w	a4,a5
     e08:	fe843783          	ld	a5,-24(s0)
     e0c:	c798                	sw	a4,8(a5)
        p += p->s.size;
     e0e:	fe843783          	ld	a5,-24(s0)
     e12:	479c                	lw	a5,8(a5)
     e14:	1782                	slli	a5,a5,0x20
     e16:	9381                	srli	a5,a5,0x20
     e18:	0792                	slli	a5,a5,0x4
     e1a:	fe843703          	ld	a4,-24(s0)
     e1e:	97ba                	add	a5,a5,a4
     e20:	fef43423          	sd	a5,-24(s0)
        p->s.size = nunits;
     e24:	fe843783          	ld	a5,-24(s0)
     e28:	fdc42703          	lw	a4,-36(s0)
     e2c:	c798                	sw	a4,8(a5)
      }
      freep = prevp;
     e2e:	00001797          	auipc	a5,0x1
     e32:	61278793          	addi	a5,a5,1554 # 2440 <freep>
     e36:	fe043703          	ld	a4,-32(s0)
     e3a:	e398                	sd	a4,0(a5)
      return (void*)(p + 1);
     e3c:	fe843783          	ld	a5,-24(s0)
     e40:	07c1                	addi	a5,a5,16
     e42:	a091                	j	e86 <malloc+0x134>
    }
    if(p == freep)
     e44:	00001797          	auipc	a5,0x1
     e48:	5fc78793          	addi	a5,a5,1532 # 2440 <freep>
     e4c:	639c                	ld	a5,0(a5)
     e4e:	fe843703          	ld	a4,-24(s0)
     e52:	02f71063          	bne	a4,a5,e72 <malloc+0x120>
      if((p = morecore(nunits)) == 0)
     e56:	fdc42783          	lw	a5,-36(s0)
     e5a:	853e                	mv	a0,a5
     e5c:	00000097          	auipc	ra,0x0
     e60:	e76080e7          	jalr	-394(ra) # cd2 <morecore>
     e64:	fea43423          	sd	a0,-24(s0)
     e68:	fe843783          	ld	a5,-24(s0)
     e6c:	e399                	bnez	a5,e72 <malloc+0x120>
        return 0;
     e6e:	4781                	li	a5,0
     e70:	a819                	j	e86 <malloc+0x134>
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
     e72:	fe843783          	ld	a5,-24(s0)
     e76:	fef43023          	sd	a5,-32(s0)
     e7a:	fe843783          	ld	a5,-24(s0)
     e7e:	639c                	ld	a5,0(a5)
     e80:	fef43423          	sd	a5,-24(s0)
    if(p->s.size >= nunits){
     e84:	b791                	j	dc8 <malloc+0x76>
  }
}
     e86:	853e                	mv	a0,a5
     e88:	70e2                	ld	ra,56(sp)
     e8a:	7442                	ld	s0,48(sp)
     e8c:	6121                	addi	sp,sp,64
     e8e:	8082                	ret

0000000000000e90 <setjmp>:
     e90:	e100                	sd	s0,0(a0)
     e92:	e504                	sd	s1,8(a0)
     e94:	01253823          	sd	s2,16(a0)
     e98:	01353c23          	sd	s3,24(a0)
     e9c:	03453023          	sd	s4,32(a0)
     ea0:	03553423          	sd	s5,40(a0)
     ea4:	03653823          	sd	s6,48(a0)
     ea8:	03753c23          	sd	s7,56(a0)
     eac:	05853023          	sd	s8,64(a0)
     eb0:	05953423          	sd	s9,72(a0)
     eb4:	05a53823          	sd	s10,80(a0)
     eb8:	05b53c23          	sd	s11,88(a0)
     ebc:	06153023          	sd	ra,96(a0)
     ec0:	06253423          	sd	sp,104(a0)
     ec4:	4501                	li	a0,0
     ec6:	8082                	ret

0000000000000ec8 <longjmp>:
     ec8:	6100                	ld	s0,0(a0)
     eca:	6504                	ld	s1,8(a0)
     ecc:	01053903          	ld	s2,16(a0)
     ed0:	01853983          	ld	s3,24(a0)
     ed4:	02053a03          	ld	s4,32(a0)
     ed8:	02853a83          	ld	s5,40(a0)
     edc:	03053b03          	ld	s6,48(a0)
     ee0:	03853b83          	ld	s7,56(a0)
     ee4:	04053c03          	ld	s8,64(a0)
     ee8:	04853c83          	ld	s9,72(a0)
     eec:	05053d03          	ld	s10,80(a0)
     ef0:	05853d83          	ld	s11,88(a0)
     ef4:	06053083          	ld	ra,96(a0)
     ef8:	06853103          	ld	sp,104(a0)
     efc:	c199                	beqz	a1,f02 <longjmp_1>
     efe:	852e                	mv	a0,a1
     f00:	8082                	ret

0000000000000f02 <longjmp_1>:
     f02:	4505                	li	a0,1
     f04:	8082                	ret

0000000000000f06 <__list_add>:
  * the prev/next entries already!
  */
 static inline void __list_add(struct list_head *new_entry,
                               struct list_head *prev,
                               struct list_head *next)
 {
     f06:	7179                	addi	sp,sp,-48
     f08:	f422                	sd	s0,40(sp)
     f0a:	1800                	addi	s0,sp,48
     f0c:	fea43423          	sd	a0,-24(s0)
     f10:	feb43023          	sd	a1,-32(s0)
     f14:	fcc43c23          	sd	a2,-40(s0)
     next->prev = new_entry;
     f18:	fd843783          	ld	a5,-40(s0)
     f1c:	fe843703          	ld	a4,-24(s0)
     f20:	e798                	sd	a4,8(a5)
     new_entry->next = next;
     f22:	fe843783          	ld	a5,-24(s0)
     f26:	fd843703          	ld	a4,-40(s0)
     f2a:	e398                	sd	a4,0(a5)
     new_entry->prev = prev;
     f2c:	fe843783          	ld	a5,-24(s0)
     f30:	fe043703          	ld	a4,-32(s0)
     f34:	e798                	sd	a4,8(a5)
     prev->next = new_entry;
     f36:	fe043783          	ld	a5,-32(s0)
     f3a:	fe843703          	ld	a4,-24(s0)
     f3e:	e398                	sd	a4,0(a5)
 }
     f40:	0001                	nop
     f42:	7422                	ld	s0,40(sp)
     f44:	6145                	addi	sp,sp,48
     f46:	8082                	ret

0000000000000f48 <list_add_tail>:
  *
  * Insert a new entry before the specified head.
  * This is useful for implementing queues.
  */
 static inline void list_add_tail(struct list_head *new_entry, struct list_head *head)
 {
     f48:	1101                	addi	sp,sp,-32
     f4a:	ec06                	sd	ra,24(sp)
     f4c:	e822                	sd	s0,16(sp)
     f4e:	1000                	addi	s0,sp,32
     f50:	fea43423          	sd	a0,-24(s0)
     f54:	feb43023          	sd	a1,-32(s0)
     __list_add(new_entry, head->prev, head);
     f58:	fe043783          	ld	a5,-32(s0)
     f5c:	679c                	ld	a5,8(a5)
     f5e:	fe043603          	ld	a2,-32(s0)
     f62:	85be                	mv	a1,a5
     f64:	fe843503          	ld	a0,-24(s0)
     f68:	00000097          	auipc	ra,0x0
     f6c:	f9e080e7          	jalr	-98(ra) # f06 <__list_add>
 }
     f70:	0001                	nop
     f72:	60e2                	ld	ra,24(sp)
     f74:	6442                	ld	s0,16(sp)
     f76:	6105                	addi	sp,sp,32
     f78:	8082                	ret

0000000000000f7a <__list_del>:
  *
  * This is only for internal list manipulation where we know
  * the prev/next entries already!
  */
 static inline void __list_del(struct list_head *prev, struct list_head *next)
 {
     f7a:	1101                	addi	sp,sp,-32
     f7c:	ec22                	sd	s0,24(sp)
     f7e:	1000                	addi	s0,sp,32
     f80:	fea43423          	sd	a0,-24(s0)
     f84:	feb43023          	sd	a1,-32(s0)
     next->prev = prev;
     f88:	fe043783          	ld	a5,-32(s0)
     f8c:	fe843703          	ld	a4,-24(s0)
     f90:	e798                	sd	a4,8(a5)
     prev->next = next;
     f92:	fe843783          	ld	a5,-24(s0)
     f96:	fe043703          	ld	a4,-32(s0)
     f9a:	e398                	sd	a4,0(a5)
 }
     f9c:	0001                	nop
     f9e:	6462                	ld	s0,24(sp)
     fa0:	6105                	addi	sp,sp,32
     fa2:	8082                	ret

0000000000000fa4 <list_del>:
  * @entry: the element to delete from the list.
  * Note: list_empty on entry does not return true after this, the entry is
  * in an undefined state.
  */
 static inline void list_del(struct list_head *entry)
 {
     fa4:	1101                	addi	sp,sp,-32
     fa6:	ec06                	sd	ra,24(sp)
     fa8:	e822                	sd	s0,16(sp)
     faa:	1000                	addi	s0,sp,32
     fac:	fea43423          	sd	a0,-24(s0)
     __list_del(entry->prev, entry->next);
     fb0:	fe843783          	ld	a5,-24(s0)
     fb4:	6798                	ld	a4,8(a5)
     fb6:	fe843783          	ld	a5,-24(s0)
     fba:	639c                	ld	a5,0(a5)
     fbc:	85be                	mv	a1,a5
     fbe:	853a                	mv	a0,a4
     fc0:	00000097          	auipc	ra,0x0
     fc4:	fba080e7          	jalr	-70(ra) # f7a <__list_del>
     entry->next = LIST_POISON1;
     fc8:	fe843783          	ld	a5,-24(s0)
     fcc:	00100737          	lui	a4,0x100
     fd0:	10070713          	addi	a4,a4,256 # 100100 <__global_pointer$+0xfd518>
     fd4:	e398                	sd	a4,0(a5)
     entry->prev = LIST_POISON2;
     fd6:	fe843783          	ld	a5,-24(s0)
     fda:	00200737          	lui	a4,0x200
     fde:	20070713          	addi	a4,a4,512 # 200200 <__global_pointer$+0x1fd618>
     fe2:	e798                	sd	a4,8(a5)
 }
     fe4:	0001                	nop
     fe6:	60e2                	ld	ra,24(sp)
     fe8:	6442                	ld	s0,16(sp)
     fea:	6105                	addi	sp,sp,32
     fec:	8082                	ret

0000000000000fee <list_empty>:
 /**
  * list_empty - tests whether a list is empty
  * @head: the list to test.
  */
 static inline int list_empty(const struct list_head *head)
 {
     fee:	1101                	addi	sp,sp,-32
     ff0:	ec22                	sd	s0,24(sp)
     ff2:	1000                	addi	s0,sp,32
     ff4:	fea43423          	sd	a0,-24(s0)
     return head->next == head;
     ff8:	fe843783          	ld	a5,-24(s0)
     ffc:	639c                	ld	a5,0(a5)
     ffe:	fe843703          	ld	a4,-24(s0)
    1002:	40f707b3          	sub	a5,a4,a5
    1006:	0017b793          	seqz	a5,a5
    100a:	0ff7f793          	andi	a5,a5,255
    100e:	2781                	sext.w	a5,a5
 }
    1010:	853e                	mv	a0,a5
    1012:	6462                	ld	s0,24(sp)
    1014:	6105                	addi	sp,sp,32
    1016:	8082                	ret

0000000000001018 <thread_create>:

void __dispatch(void);
void __schedule(void);

struct thread *thread_create(void (*f)(void *), void *arg, int is_real_time, int processing_time, int period, int n)
{
    1018:	715d                	addi	sp,sp,-80
    101a:	e486                	sd	ra,72(sp)
    101c:	e0a2                	sd	s0,64(sp)
    101e:	0880                	addi	s0,sp,80
    1020:	fca43423          	sd	a0,-56(s0)
    1024:	fcb43023          	sd	a1,-64(s0)
    1028:	85b2                	mv	a1,a2
    102a:	8636                	mv	a2,a3
    102c:	86ba                	mv	a3,a4
    102e:	873e                	mv	a4,a5
    1030:	87ae                	mv	a5,a1
    1032:	faf42e23          	sw	a5,-68(s0)
    1036:	87b2                	mv	a5,a2
    1038:	faf42c23          	sw	a5,-72(s0)
    103c:	87b6                	mv	a5,a3
    103e:	faf42a23          	sw	a5,-76(s0)
    1042:	87ba                	mv	a5,a4
    1044:	faf42823          	sw	a5,-80(s0)
    static int _id = 1;
    struct thread *t = (struct thread *)malloc(sizeof(struct thread));
    1048:	08000513          	li	a0,128
    104c:	00000097          	auipc	ra,0x0
    1050:	d06080e7          	jalr	-762(ra) # d52 <malloc>
    1054:	fea43423          	sd	a0,-24(s0)
    unsigned long new_stack_p;
    unsigned long new_stack;
    new_stack = (unsigned long)malloc(sizeof(unsigned long) * 0x200);
    1058:	6505                	lui	a0,0x1
    105a:	00000097          	auipc	ra,0x0
    105e:	cf8080e7          	jalr	-776(ra) # d52 <malloc>
    1062:	87aa                	mv	a5,a0
    1064:	fef43023          	sd	a5,-32(s0)
    new_stack_p = new_stack + 0x200 * 8 - 0x2 * 8;
    1068:	fe043703          	ld	a4,-32(s0)
    106c:	6785                	lui	a5,0x1
    106e:	17c1                	addi	a5,a5,-16
    1070:	97ba                	add	a5,a5,a4
    1072:	fcf43c23          	sd	a5,-40(s0)
    t->fp = f;
    1076:	fe843783          	ld	a5,-24(s0)
    107a:	fc843703          	ld	a4,-56(s0)
    107e:	e398                	sd	a4,0(a5)
    t->arg = arg;
    1080:	fe843783          	ld	a5,-24(s0)
    1084:	fc043703          	ld	a4,-64(s0)
    1088:	e798                	sd	a4,8(a5)
    t->ID = _id++;
    108a:	00001797          	auipc	a5,0x1
    108e:	39a78793          	addi	a5,a5,922 # 2424 <_id.1241>
    1092:	439c                	lw	a5,0(a5)
    1094:	0017871b          	addiw	a4,a5,1
    1098:	0007069b          	sext.w	a3,a4
    109c:	00001717          	auipc	a4,0x1
    10a0:	38870713          	addi	a4,a4,904 # 2424 <_id.1241>
    10a4:	c314                	sw	a3,0(a4)
    10a6:	fe843703          	ld	a4,-24(s0)
    10aa:	df5c                	sw	a5,60(a4)
    t->buf_set = 0;
    10ac:	fe843783          	ld	a5,-24(s0)
    10b0:	0207a023          	sw	zero,32(a5)
    t->stack = (void *)new_stack;
    10b4:	fe043703          	ld	a4,-32(s0)
    10b8:	fe843783          	ld	a5,-24(s0)
    10bc:	eb98                	sd	a4,16(a5)
    t->stack_p = (void *)new_stack_p;
    10be:	fd843703          	ld	a4,-40(s0)
    10c2:	fe843783          	ld	a5,-24(s0)
    10c6:	ef98                	sd	a4,24(a5)

    t->processing_time = processing_time;
    10c8:	fe843783          	ld	a5,-24(s0)
    10cc:	fb842703          	lw	a4,-72(s0)
    10d0:	c3f8                	sw	a4,68(a5)
    t->period = period;
    10d2:	fe843783          	ld	a5,-24(s0)
    10d6:	fb442703          	lw	a4,-76(s0)
    10da:	c7f8                	sw	a4,76(a5)
    t->deadline = period;
    10dc:	fe843783          	ld	a5,-24(s0)
    10e0:	fb442703          	lw	a4,-76(s0)
    10e4:	c7b8                	sw	a4,72(a5)
    t->n = n;
    10e6:	fe843783          	ld	a5,-24(s0)
    10ea:	fb042703          	lw	a4,-80(s0)
    10ee:	cbb8                	sw	a4,80(a5)
    t->is_real_time = is_real_time;
    10f0:	fe843783          	ld	a5,-24(s0)
    10f4:	fbc42703          	lw	a4,-68(s0)
    10f8:	c3b8                	sw	a4,64(a5)
    t->remaining_time = processing_time;
    10fa:	fe843783          	ld	a5,-24(s0)
    10fe:	fb842703          	lw	a4,-72(s0)
    1102:	cbf8                	sw	a4,84(a5)
    t->current_deadline = 0;
    1104:	fe843783          	ld	a5,-24(s0)
    1108:	0407ac23          	sw	zero,88(a5)
    t->priority = 100;
    110c:	fe843783          	ld	a5,-24(s0)
    1110:	06400713          	li	a4,100
    1114:	cff8                	sw	a4,92(a5)
    t->arrival_time = 30000;
    1116:	fe843783          	ld	a5,-24(s0)
    111a:	671d                	lui	a4,0x7
    111c:	5307071b          	addiw	a4,a4,1328
    1120:	d3b8                	sw	a4,96(a5)
    
    return t;
    1122:	fe843783          	ld	a5,-24(s0)
}
    1126:	853e                	mv	a0,a5
    1128:	60a6                	ld	ra,72(sp)
    112a:	6406                	ld	s0,64(sp)
    112c:	6161                	addi	sp,sp,80
    112e:	8082                	ret

0000000000001130 <thread_set_priority>:

void thread_set_priority(struct thread *t, int priority)
{
    1130:	1101                	addi	sp,sp,-32
    1132:	ec22                	sd	s0,24(sp)
    1134:	1000                	addi	s0,sp,32
    1136:	fea43423          	sd	a0,-24(s0)
    113a:	87ae                	mv	a5,a1
    113c:	fef42223          	sw	a5,-28(s0)
    t->priority = priority;
    1140:	fe843783          	ld	a5,-24(s0)
    1144:	fe442703          	lw	a4,-28(s0)
    1148:	cff8                	sw	a4,92(a5)
}
    114a:	0001                	nop
    114c:	6462                	ld	s0,24(sp)
    114e:	6105                	addi	sp,sp,32
    1150:	8082                	ret

0000000000001152 <init_thread_cbs>:
void init_thread_cbs(struct thread *t, int budget, int is_hard_rt)
{
    1152:	1101                	addi	sp,sp,-32
    1154:	ec22                	sd	s0,24(sp)
    1156:	1000                	addi	s0,sp,32
    1158:	fea43423          	sd	a0,-24(s0)
    115c:	87ae                	mv	a5,a1
    115e:	8732                	mv	a4,a2
    1160:	fef42223          	sw	a5,-28(s0)
    1164:	87ba                	mv	a5,a4
    1166:	fef42023          	sw	a5,-32(s0)
    t->cbs.budget = budget;
    116a:	fe843783          	ld	a5,-24(s0)
    116e:	fe442703          	lw	a4,-28(s0)
    1172:	d3f8                	sw	a4,100(a5)
    t->cbs.remaining_budget = budget; 
    1174:	fe843783          	ld	a5,-24(s0)
    1178:	fe442703          	lw	a4,-28(s0)
    117c:	d7b8                	sw	a4,104(a5)
    t->cbs.is_hard_rt = is_hard_rt;
    117e:	fe843783          	ld	a5,-24(s0)
    1182:	fe042703          	lw	a4,-32(s0)
    1186:	d7f8                	sw	a4,108(a5)
    t->cbs.is_throttled = 0;
    1188:	fe843783          	ld	a5,-24(s0)
    118c:	0607a823          	sw	zero,112(a5)
    t->cbs.throttled_arrived_time = 0;
    1190:	fe843783          	ld	a5,-24(s0)
    1194:	0607aa23          	sw	zero,116(a5)
    t->cbs.throttle_new_deadline = 0;
    1198:	fe843783          	ld	a5,-24(s0)
    119c:	0607ac23          	sw	zero,120(a5)
    t->cbs.remaining_time_at_throttle = 0;
    11a0:	fe843783          	ld	a5,-24(s0)
    11a4:	0607ae23          	sw	zero,124(a5)
}
    11a8:	0001                	nop
    11aa:	6462                	ld	s0,24(sp)
    11ac:	6105                	addi	sp,sp,32
    11ae:	8082                	ret

00000000000011b0 <thread_add_at>:
void thread_add_at(struct thread *t, int arrival_time)
{
    11b0:	7179                	addi	sp,sp,-48
    11b2:	f406                	sd	ra,40(sp)
    11b4:	f022                	sd	s0,32(sp)
    11b6:	1800                	addi	s0,sp,48
    11b8:	fca43c23          	sd	a0,-40(s0)
    11bc:	87ae                	mv	a5,a1
    11be:	fcf42a23          	sw	a5,-44(s0)
    struct release_queue_entry *new_entry = (struct release_queue_entry *)malloc(sizeof(struct release_queue_entry));
    11c2:	02000513          	li	a0,32
    11c6:	00000097          	auipc	ra,0x0
    11ca:	b8c080e7          	jalr	-1140(ra) # d52 <malloc>
    11ce:	fea43423          	sd	a0,-24(s0)
    new_entry->thrd = t;
    11d2:	fe843783          	ld	a5,-24(s0)
    11d6:	fd843703          	ld	a4,-40(s0)
    11da:	e398                	sd	a4,0(a5)
    new_entry->release_time = arrival_time;
    11dc:	fe843783          	ld	a5,-24(s0)
    11e0:	fd442703          	lw	a4,-44(s0)
    11e4:	cf98                	sw	a4,24(a5)
    t->arrival_time = arrival_time;
    11e6:	fd843783          	ld	a5,-40(s0)
    11ea:	fd442703          	lw	a4,-44(s0)
    11ee:	d3b8                	sw	a4,96(a5)

    // t->remaining_time = t->processing_time;
    if (t->is_real_time) {
    11f0:	fd843783          	ld	a5,-40(s0)
    11f4:	43bc                	lw	a5,64(a5)
    11f6:	cf81                	beqz	a5,120e <thread_add_at+0x5e>
        t->current_deadline = arrival_time + t->deadline;
    11f8:	fd843783          	ld	a5,-40(s0)
    11fc:	47bc                	lw	a5,72(a5)
    11fe:	fd442703          	lw	a4,-44(s0)
    1202:	9fb9                	addw	a5,a5,a4
    1204:	0007871b          	sext.w	a4,a5
    1208:	fd843783          	ld	a5,-40(s0)
    120c:	cfb8                	sw	a4,88(a5)
    }
    list_add_tail(&new_entry->thread_list, &release_queue);
    120e:	fe843783          	ld	a5,-24(s0)
    1212:	07a1                	addi	a5,a5,8
    1214:	00001597          	auipc	a1,0x1
    1218:	1fc58593          	addi	a1,a1,508 # 2410 <release_queue>
    121c:	853e                	mv	a0,a5
    121e:	00000097          	auipc	ra,0x0
    1222:	d2a080e7          	jalr	-726(ra) # f48 <list_add_tail>
}
    1226:	0001                	nop
    1228:	70a2                	ld	ra,40(sp)
    122a:	7402                	ld	s0,32(sp)
    122c:	6145                	addi	sp,sp,48
    122e:	8082                	ret

0000000000001230 <__release>:

void __release()
{
    1230:	7139                	addi	sp,sp,-64
    1232:	fc06                	sd	ra,56(sp)
    1234:	f822                	sd	s0,48(sp)
    1236:	0080                	addi	s0,sp,64
    struct release_queue_entry *cur, *nxt;
    list_for_each_entry_safe(cur, nxt, &release_queue, thread_list) {
    1238:	00001797          	auipc	a5,0x1
    123c:	1d878793          	addi	a5,a5,472 # 2410 <release_queue>
    1240:	639c                	ld	a5,0(a5)
    1242:	fcf43c23          	sd	a5,-40(s0)
    1246:	fd843783          	ld	a5,-40(s0)
    124a:	17e1                	addi	a5,a5,-8
    124c:	fef43423          	sd	a5,-24(s0)
    1250:	fe843783          	ld	a5,-24(s0)
    1254:	679c                	ld	a5,8(a5)
    1256:	fcf43823          	sd	a5,-48(s0)
    125a:	fd043783          	ld	a5,-48(s0)
    125e:	17e1                	addi	a5,a5,-8
    1260:	fef43023          	sd	a5,-32(s0)
    1264:	a0e9                	j	132e <__release+0xfe>
        if (threading_system_time >= cur->release_time) {
    1266:	fe843783          	ld	a5,-24(s0)
    126a:	4f98                	lw	a4,24(a5)
    126c:	00001797          	auipc	a5,0x1
    1270:	1e478793          	addi	a5,a5,484 # 2450 <threading_system_time>
    1274:	439c                	lw	a5,0(a5)
    1276:	08e7ce63          	blt	a5,a4,1312 <__release+0xe2>
            if(cur->thrd->cbs.is_throttled){
    127a:	fe843783          	ld	a5,-24(s0)
    127e:	639c                	ld	a5,0(a5)
    1280:	5bbc                	lw	a5,112(a5)
    1282:	c79d                	beqz	a5,12b0 <__release+0x80>
                cur->thrd->cbs.is_throttled = 0;
    1284:	fe843783          	ld	a5,-24(s0)
    1288:	639c                	ld	a5,0(a5)
    128a:	0607a823          	sw	zero,112(a5)
                cur->thrd->cbs.remaining_budget = cur->thrd->cbs.budget;
    128e:	fe843783          	ld	a5,-24(s0)
    1292:	6398                	ld	a4,0(a5)
    1294:	fe843783          	ld	a5,-24(s0)
    1298:	639c                	ld	a5,0(a5)
    129a:	5378                	lw	a4,100(a4)
    129c:	d7b8                	sw	a4,104(a5)
                cur->thrd->remaining_time = cur->thrd->cbs.remaining_time_at_throttle;
    129e:	fe843783          	ld	a5,-24(s0)
    12a2:	6398                	ld	a4,0(a5)
    12a4:	fe843783          	ld	a5,-24(s0)
    12a8:	639c                	ld	a5,0(a5)
    12aa:	5f78                	lw	a4,124(a4)
    12ac:	cbf8                	sw	a4,84(a5)
    12ae:	a809                	j	12c0 <__release+0x90>
            }else{
                cur->thrd->remaining_time = cur->thrd->processing_time;
    12b0:	fe843783          	ld	a5,-24(s0)
    12b4:	6398                	ld	a4,0(a5)
    12b6:	fe843783          	ld	a5,-24(s0)
    12ba:	639c                	ld	a5,0(a5)
    12bc:	4378                	lw	a4,68(a4)
    12be:	cbf8                	sw	a4,84(a5)
            }
            cur->thrd->current_deadline = cur->release_time + cur->thrd->deadline;
    12c0:	fe843783          	ld	a5,-24(s0)
    12c4:	4f94                	lw	a3,24(a5)
    12c6:	fe843783          	ld	a5,-24(s0)
    12ca:	639c                	ld	a5,0(a5)
    12cc:	47b8                	lw	a4,72(a5)
    12ce:	fe843783          	ld	a5,-24(s0)
    12d2:	639c                	ld	a5,0(a5)
    12d4:	9f35                	addw	a4,a4,a3
    12d6:	2701                	sext.w	a4,a4
    12d8:	cfb8                	sw	a4,88(a5)
            list_add_tail(&cur->thrd->thread_list, &run_queue);
    12da:	fe843783          	ld	a5,-24(s0)
    12de:	639c                	ld	a5,0(a5)
    12e0:	02878793          	addi	a5,a5,40
    12e4:	00001597          	auipc	a1,0x1
    12e8:	11c58593          	addi	a1,a1,284 # 2400 <run_queue>
    12ec:	853e                	mv	a0,a5
    12ee:	00000097          	auipc	ra,0x0
    12f2:	c5a080e7          	jalr	-934(ra) # f48 <list_add_tail>
            list_del(&cur->thread_list);
    12f6:	fe843783          	ld	a5,-24(s0)
    12fa:	07a1                	addi	a5,a5,8
    12fc:	853e                	mv	a0,a5
    12fe:	00000097          	auipc	ra,0x0
    1302:	ca6080e7          	jalr	-858(ra) # fa4 <list_del>
            free(cur);
    1306:	fe843503          	ld	a0,-24(s0)
    130a:	00000097          	auipc	ra,0x0
    130e:	8a6080e7          	jalr	-1882(ra) # bb0 <free>
    list_for_each_entry_safe(cur, nxt, &release_queue, thread_list) {
    1312:	fe043783          	ld	a5,-32(s0)
    1316:	fef43423          	sd	a5,-24(s0)
    131a:	fe043783          	ld	a5,-32(s0)
    131e:	679c                	ld	a5,8(a5)
    1320:	fcf43423          	sd	a5,-56(s0)
    1324:	fc843783          	ld	a5,-56(s0)
    1328:	17e1                	addi	a5,a5,-8
    132a:	fef43023          	sd	a5,-32(s0)
    132e:	fe843783          	ld	a5,-24(s0)
    1332:	00878713          	addi	a4,a5,8
    1336:	00001797          	auipc	a5,0x1
    133a:	0da78793          	addi	a5,a5,218 # 2410 <release_queue>
    133e:	f2f714e3          	bne	a4,a5,1266 <__release+0x36>
        }
    }
}
    1342:	0001                	nop
    1344:	0001                	nop
    1346:	70e2                	ld	ra,56(sp)
    1348:	7442                	ld	s0,48(sp)
    134a:	6121                	addi	sp,sp,64
    134c:	8082                	ret

000000000000134e <__thread_exit>:

void __thread_exit(struct thread *to_remove)
{
    134e:	1101                	addi	sp,sp,-32
    1350:	ec06                	sd	ra,24(sp)
    1352:	e822                	sd	s0,16(sp)
    1354:	1000                	addi	s0,sp,32
    1356:	fea43423          	sd	a0,-24(s0)
    current = to_remove->thread_list.prev;
    135a:	fe843783          	ld	a5,-24(s0)
    135e:	7b98                	ld	a4,48(a5)
    1360:	00001797          	auipc	a5,0x1
    1364:	0e878793          	addi	a5,a5,232 # 2448 <current>
    1368:	e398                	sd	a4,0(a5)
    list_del(&to_remove->thread_list);
    136a:	fe843783          	ld	a5,-24(s0)
    136e:	02878793          	addi	a5,a5,40
    1372:	853e                	mv	a0,a5
    1374:	00000097          	auipc	ra,0x0
    1378:	c30080e7          	jalr	-976(ra) # fa4 <list_del>

    free(to_remove->stack);
    137c:	fe843783          	ld	a5,-24(s0)
    1380:	6b9c                	ld	a5,16(a5)
    1382:	853e                	mv	a0,a5
    1384:	00000097          	auipc	ra,0x0
    1388:	82c080e7          	jalr	-2004(ra) # bb0 <free>
    free(to_remove);
    138c:	fe843503          	ld	a0,-24(s0)
    1390:	00000097          	auipc	ra,0x0
    1394:	820080e7          	jalr	-2016(ra) # bb0 <free>

    __schedule();
    1398:	00000097          	auipc	ra,0x0
    139c:	5e8080e7          	jalr	1512(ra) # 1980 <__schedule>
    __dispatch();
    13a0:	00000097          	auipc	ra,0x0
    13a4:	416080e7          	jalr	1046(ra) # 17b6 <__dispatch>
    thrdresume(main_thrd_id);
    13a8:	00001797          	auipc	a5,0x1
    13ac:	07878793          	addi	a5,a5,120 # 2420 <main_thrd_id>
    13b0:	439c                	lw	a5,0(a5)
    13b2:	853e                	mv	a0,a5
    13b4:	fffff097          	auipc	ra,0xfffff
    13b8:	30e080e7          	jalr	782(ra) # 6c2 <thrdresume>
}
    13bc:	0001                	nop
    13be:	60e2                	ld	ra,24(sp)
    13c0:	6442                	ld	s0,16(sp)
    13c2:	6105                	addi	sp,sp,32
    13c4:	8082                	ret

00000000000013c6 <thread_exit>:

void thread_exit(void)
{
    13c6:	7179                	addi	sp,sp,-48
    13c8:	f406                	sd	ra,40(sp)
    13ca:	f022                	sd	s0,32(sp)
    13cc:	1800                	addi	s0,sp,48
    if (current == &run_queue) {
    13ce:	00001797          	auipc	a5,0x1
    13d2:	07a78793          	addi	a5,a5,122 # 2448 <current>
    13d6:	6398                	ld	a4,0(a5)
    13d8:	00001797          	auipc	a5,0x1
    13dc:	02878793          	addi	a5,a5,40 # 2400 <run_queue>
    13e0:	02f71063          	bne	a4,a5,1400 <thread_exit+0x3a>
        fprintf(2, "[FATAL] thread_exit is called on a nonexistent thread\n");
    13e4:	00001597          	auipc	a1,0x1
    13e8:	d8c58593          	addi	a1,a1,-628 # 2170 <schedule_edf_cbs+0x51a>
    13ec:	4509                	li	a0,2
    13ee:	fffff097          	auipc	ra,0xfffff
    13f2:	71a080e7          	jalr	1818(ra) # b08 <fprintf>
        exit(1);
    13f6:	4505                	li	a0,1
    13f8:	fffff097          	auipc	ra,0xfffff
    13fc:	222080e7          	jalr	546(ra) # 61a <exit>
    }

    struct thread *to_remove = list_entry(current, struct thread, thread_list);
    1400:	00001797          	auipc	a5,0x1
    1404:	04878793          	addi	a5,a5,72 # 2448 <current>
    1408:	639c                	ld	a5,0(a5)
    140a:	fef43423          	sd	a5,-24(s0)
    140e:	fe843783          	ld	a5,-24(s0)
    1412:	fd878793          	addi	a5,a5,-40
    1416:	fef43023          	sd	a5,-32(s0)
    int consume_ticks = cancelthrdstop(to_remove->thrdstop_context_id, 1);
    141a:	fe043783          	ld	a5,-32(s0)
    141e:	5f9c                	lw	a5,56(a5)
    1420:	4585                	li	a1,1
    1422:	853e                	mv	a0,a5
    1424:	fffff097          	auipc	ra,0xfffff
    1428:	2a6080e7          	jalr	678(ra) # 6ca <cancelthrdstop>
    142c:	87aa                	mv	a5,a0
    142e:	fcf42e23          	sw	a5,-36(s0)
    threading_system_time += consume_ticks;
    1432:	00001797          	auipc	a5,0x1
    1436:	01e78793          	addi	a5,a5,30 # 2450 <threading_system_time>
    143a:	439c                	lw	a5,0(a5)
    143c:	fdc42703          	lw	a4,-36(s0)
    1440:	9fb9                	addw	a5,a5,a4
    1442:	0007871b          	sext.w	a4,a5
    1446:	00001797          	auipc	a5,0x1
    144a:	00a78793          	addi	a5,a5,10 # 2450 <threading_system_time>
    144e:	c398                	sw	a4,0(a5)

    __release();
    1450:	00000097          	auipc	ra,0x0
    1454:	de0080e7          	jalr	-544(ra) # 1230 <__release>
    __thread_exit(to_remove);
    1458:	fe043503          	ld	a0,-32(s0)
    145c:	00000097          	auipc	ra,0x0
    1460:	ef2080e7          	jalr	-270(ra) # 134e <__thread_exit>
}
    1464:	0001                	nop
    1466:	70a2                	ld	ra,40(sp)
    1468:	7402                	ld	s0,32(sp)
    146a:	6145                	addi	sp,sp,48
    146c:	8082                	ret

000000000000146e <__finish_current>:

void __finish_current()
{
    146e:	7179                	addi	sp,sp,-48
    1470:	f406                	sd	ra,40(sp)
    1472:	f022                	sd	s0,32(sp)
    1474:	1800                	addi	s0,sp,48
    struct thread *current_thread = list_entry(current, struct thread, thread_list);
    1476:	00001797          	auipc	a5,0x1
    147a:	fd278793          	addi	a5,a5,-46 # 2448 <current>
    147e:	639c                	ld	a5,0(a5)
    1480:	fef43423          	sd	a5,-24(s0)
    1484:	fe843783          	ld	a5,-24(s0)
    1488:	fd878793          	addi	a5,a5,-40
    148c:	fef43023          	sd	a5,-32(s0)
    --current_thread->n;
    1490:	fe043783          	ld	a5,-32(s0)
    1494:	4bbc                	lw	a5,80(a5)
    1496:	37fd                	addiw	a5,a5,-1
    1498:	0007871b          	sext.w	a4,a5
    149c:	fe043783          	ld	a5,-32(s0)
    14a0:	cbb8                	sw	a4,80(a5)

    printf("thread#%d finish at %d\n",
    14a2:	fe043783          	ld	a5,-32(s0)
    14a6:	5fd8                	lw	a4,60(a5)
    14a8:	00001797          	auipc	a5,0x1
    14ac:	fa878793          	addi	a5,a5,-88 # 2450 <threading_system_time>
    14b0:	4390                	lw	a2,0(a5)
    14b2:	fe043783          	ld	a5,-32(s0)
    14b6:	4bbc                	lw	a5,80(a5)
    14b8:	86be                	mv	a3,a5
    14ba:	85ba                	mv	a1,a4
    14bc:	00001517          	auipc	a0,0x1
    14c0:	cec50513          	addi	a0,a0,-788 # 21a8 <schedule_edf_cbs+0x552>
    14c4:	fffff097          	auipc	ra,0xfffff
    14c8:	69c080e7          	jalr	1692(ra) # b60 <printf>
           current_thread->ID, threading_system_time, current_thread->n);

    if (current_thread->n > 0) {
    14cc:	fe043783          	ld	a5,-32(s0)
    14d0:	4bbc                	lw	a5,80(a5)
    14d2:	04f05563          	blez	a5,151c <__finish_current+0xae>
        struct list_head *to_remove = current;
    14d6:	00001797          	auipc	a5,0x1
    14da:	f7278793          	addi	a5,a5,-142 # 2448 <current>
    14de:	639c                	ld	a5,0(a5)
    14e0:	fcf43c23          	sd	a5,-40(s0)
        current = current->prev;
    14e4:	00001797          	auipc	a5,0x1
    14e8:	f6478793          	addi	a5,a5,-156 # 2448 <current>
    14ec:	639c                	ld	a5,0(a5)
    14ee:	6798                	ld	a4,8(a5)
    14f0:	00001797          	auipc	a5,0x1
    14f4:	f5878793          	addi	a5,a5,-168 # 2448 <current>
    14f8:	e398                	sd	a4,0(a5)
        list_del(to_remove);
    14fa:	fd843503          	ld	a0,-40(s0)
    14fe:	00000097          	auipc	ra,0x0
    1502:	aa6080e7          	jalr	-1370(ra) # fa4 <list_del>
        thread_add_at(current_thread, current_thread->current_deadline);
    1506:	fe043783          	ld	a5,-32(s0)
    150a:	4fbc                	lw	a5,88(a5)
    150c:	85be                	mv	a1,a5
    150e:	fe043503          	ld	a0,-32(s0)
    1512:	00000097          	auipc	ra,0x0
    1516:	c9e080e7          	jalr	-866(ra) # 11b0 <thread_add_at>
    } else {
        __thread_exit(current_thread);
    }
}
    151a:	a039                	j	1528 <__finish_current+0xba>
        __thread_exit(current_thread);
    151c:	fe043503          	ld	a0,-32(s0)
    1520:	00000097          	auipc	ra,0x0
    1524:	e2e080e7          	jalr	-466(ra) # 134e <__thread_exit>
}
    1528:	0001                	nop
    152a:	70a2                	ld	ra,40(sp)
    152c:	7402                	ld	s0,32(sp)
    152e:	6145                	addi	sp,sp,48
    1530:	8082                	ret

0000000000001532 <__rt_finish_current>:
void __rt_finish_current()
{
    1532:	7179                	addi	sp,sp,-48
    1534:	f406                	sd	ra,40(sp)
    1536:	f022                	sd	s0,32(sp)
    1538:	1800                	addi	s0,sp,48
    struct thread *current_thread = list_entry(current, struct thread, thread_list);
    153a:	00001797          	auipc	a5,0x1
    153e:	f0e78793          	addi	a5,a5,-242 # 2448 <current>
    1542:	639c                	ld	a5,0(a5)
    1544:	fef43423          	sd	a5,-24(s0)
    1548:	fe843783          	ld	a5,-24(s0)
    154c:	fd878793          	addi	a5,a5,-40
    1550:	fef43023          	sd	a5,-32(s0)
    --current_thread->n;
    1554:	fe043783          	ld	a5,-32(s0)
    1558:	4bbc                	lw	a5,80(a5)
    155a:	37fd                	addiw	a5,a5,-1
    155c:	0007871b          	sext.w	a4,a5
    1560:	fe043783          	ld	a5,-32(s0)
    1564:	cbb8                	sw	a4,80(a5)

    printf("thread#%d finish one cycle at %d: %d cycles left\n",
    1566:	fe043783          	ld	a5,-32(s0)
    156a:	5fd8                	lw	a4,60(a5)
    156c:	00001797          	auipc	a5,0x1
    1570:	ee478793          	addi	a5,a5,-284 # 2450 <threading_system_time>
    1574:	4390                	lw	a2,0(a5)
    1576:	fe043783          	ld	a5,-32(s0)
    157a:	4bbc                	lw	a5,80(a5)
    157c:	86be                	mv	a3,a5
    157e:	85ba                	mv	a1,a4
    1580:	00001517          	auipc	a0,0x1
    1584:	c4050513          	addi	a0,a0,-960 # 21c0 <schedule_edf_cbs+0x56a>
    1588:	fffff097          	auipc	ra,0xfffff
    158c:	5d8080e7          	jalr	1496(ra) # b60 <printf>
           current_thread->ID, threading_system_time, current_thread->n);

    if (current_thread->n > 0) {
    1590:	fe043783          	ld	a5,-32(s0)
    1594:	4bbc                	lw	a5,80(a5)
    1596:	04f05f63          	blez	a5,15f4 <__rt_finish_current+0xc2>
        struct list_head *to_remove = current;
    159a:	00001797          	auipc	a5,0x1
    159e:	eae78793          	addi	a5,a5,-338 # 2448 <current>
    15a2:	639c                	ld	a5,0(a5)
    15a4:	fcf43c23          	sd	a5,-40(s0)
        current = current->prev;
    15a8:	00001797          	auipc	a5,0x1
    15ac:	ea078793          	addi	a5,a5,-352 # 2448 <current>
    15b0:	639c                	ld	a5,0(a5)
    15b2:	6798                	ld	a4,8(a5)
    15b4:	00001797          	auipc	a5,0x1
    15b8:	e9478793          	addi	a5,a5,-364 # 2448 <current>
    15bc:	e398                	sd	a4,0(a5)
        list_del(to_remove);
    15be:	fd843503          	ld	a0,-40(s0)
    15c2:	00000097          	auipc	ra,0x0
    15c6:	9e2080e7          	jalr	-1566(ra) # fa4 <list_del>
        thread_add_at(current_thread, current_thread->current_deadline);
    15ca:	fe043783          	ld	a5,-32(s0)
    15ce:	4fbc                	lw	a5,88(a5)
    15d0:	85be                	mv	a1,a5
    15d2:	fe043503          	ld	a0,-32(s0)
    15d6:	00000097          	auipc	ra,0x0
    15da:	bda080e7          	jalr	-1062(ra) # 11b0 <thread_add_at>
        if (!current_thread->cbs.is_hard_rt) {
    15de:	fe043783          	ld	a5,-32(s0)
    15e2:	57fc                	lw	a5,108(a5)
    15e4:	ef91                	bnez	a5,1600 <__rt_finish_current+0xce>
            current_thread->cbs.remaining_budget = current_thread->cbs.budget;
    15e6:	fe043783          	ld	a5,-32(s0)
    15ea:	53f8                	lw	a4,100(a5)
    15ec:	fe043783          	ld	a5,-32(s0)
    15f0:	d7b8                	sw	a4,104(a5)
        }
    } else {
        __thread_exit(current_thread);
    }
}
    15f2:	a039                	j	1600 <__rt_finish_current+0xce>
        __thread_exit(current_thread);
    15f4:	fe043503          	ld	a0,-32(s0)
    15f8:	00000097          	auipc	ra,0x0
    15fc:	d56080e7          	jalr	-682(ra) # 134e <__thread_exit>
}
    1600:	0001                	nop
    1602:	70a2                	ld	ra,40(sp)
    1604:	7402                	ld	s0,32(sp)
    1606:	6145                	addi	sp,sp,48
    1608:	8082                	ret

000000000000160a <switch_handler>:

void switch_handler(void *arg)
{
    160a:	7139                	addi	sp,sp,-64
    160c:	fc06                	sd	ra,56(sp)
    160e:	f822                	sd	s0,48(sp)
    1610:	0080                	addi	s0,sp,64
    1612:	fca43423          	sd	a0,-56(s0)
    uint64 elapsed_time = (uint64)arg;
    1616:	fc843783          	ld	a5,-56(s0)
    161a:	fef43423          	sd	a5,-24(s0)
    struct thread *current_thread = list_entry(current, struct thread, thread_list);
    161e:	00001797          	auipc	a5,0x1
    1622:	e2a78793          	addi	a5,a5,-470 # 2448 <current>
    1626:	639c                	ld	a5,0(a5)
    1628:	fef43023          	sd	a5,-32(s0)
    162c:	fe043783          	ld	a5,-32(s0)
    1630:	fd878793          	addi	a5,a5,-40
    1634:	fcf43c23          	sd	a5,-40(s0)

    threading_system_time += elapsed_time;
    1638:	fe843783          	ld	a5,-24(s0)
    163c:	0007871b          	sext.w	a4,a5
    1640:	00001797          	auipc	a5,0x1
    1644:	e1078793          	addi	a5,a5,-496 # 2450 <threading_system_time>
    1648:	439c                	lw	a5,0(a5)
    164a:	2781                	sext.w	a5,a5
    164c:	9fb9                	addw	a5,a5,a4
    164e:	2781                	sext.w	a5,a5
    1650:	0007871b          	sext.w	a4,a5
    1654:	00001797          	auipc	a5,0x1
    1658:	dfc78793          	addi	a5,a5,-516 # 2450 <threading_system_time>
    165c:	c398                	sw	a4,0(a5)
     __release();
    165e:	00000097          	auipc	ra,0x0
    1662:	bd2080e7          	jalr	-1070(ra) # 1230 <__release>
    current_thread->remaining_time -= elapsed_time;
    1666:	fd843783          	ld	a5,-40(s0)
    166a:	4bfc                	lw	a5,84(a5)
    166c:	0007871b          	sext.w	a4,a5
    1670:	fe843783          	ld	a5,-24(s0)
    1674:	2781                	sext.w	a5,a5
    1676:	40f707bb          	subw	a5,a4,a5
    167a:	2781                	sext.w	a5,a5
    167c:	0007871b          	sext.w	a4,a5
    1680:	fd843783          	ld	a5,-40(s0)
    1684:	cbf8                	sw	a4,84(a5)
    if (!current_thread->cbs.is_hard_rt) {
    1686:	fd843783          	ld	a5,-40(s0)
    168a:	57fc                	lw	a5,108(a5)
    168c:	e38d                	bnez	a5,16ae <switch_handler+0xa4>
        current_thread->cbs.remaining_budget -= elapsed_time;
    168e:	fd843783          	ld	a5,-40(s0)
    1692:	57bc                	lw	a5,104(a5)
    1694:	0007871b          	sext.w	a4,a5
    1698:	fe843783          	ld	a5,-24(s0)
    169c:	2781                	sext.w	a5,a5
    169e:	40f707bb          	subw	a5,a4,a5
    16a2:	2781                	sext.w	a5,a5
    16a4:	0007871b          	sext.w	a4,a5
    16a8:	fd843783          	ld	a5,-40(s0)
    16ac:	d7b8                	sw	a4,104(a5)
    }
    if (current_thread->is_real_time)
    16ae:	fd843783          	ld	a5,-40(s0)
    16b2:	43bc                	lw	a5,64(a5)
    16b4:	c3ad                	beqz	a5,1716 <switch_handler+0x10c>
        if (threading_system_time > current_thread->current_deadline || 
    16b6:	fd843783          	ld	a5,-40(s0)
    16ba:	4fb8                	lw	a4,88(a5)
    16bc:	00001797          	auipc	a5,0x1
    16c0:	d9478793          	addi	a5,a5,-620 # 2450 <threading_system_time>
    16c4:	439c                	lw	a5,0(a5)
    16c6:	02f74163          	blt	a4,a5,16e8 <switch_handler+0xde>
            (threading_system_time == current_thread->current_deadline && current_thread->remaining_time > 0)) {
    16ca:	fd843783          	ld	a5,-40(s0)
    16ce:	4fb8                	lw	a4,88(a5)
    16d0:	00001797          	auipc	a5,0x1
    16d4:	d8078793          	addi	a5,a5,-640 # 2450 <threading_system_time>
    16d8:	439c                	lw	a5,0(a5)
        if (threading_system_time > current_thread->current_deadline || 
    16da:	02f71e63          	bne	a4,a5,1716 <switch_handler+0x10c>
            (threading_system_time == current_thread->current_deadline && current_thread->remaining_time > 0)) {
    16de:	fd843783          	ld	a5,-40(s0)
    16e2:	4bfc                	lw	a5,84(a5)
    16e4:	02f05963          	blez	a5,1716 <switch_handler+0x10c>
            printf("thread#%d misses a deadline at %d in swicth\n", current_thread->ID, threading_system_time);
    16e8:	fd843783          	ld	a5,-40(s0)
    16ec:	5fd8                	lw	a4,60(a5)
    16ee:	00001797          	auipc	a5,0x1
    16f2:	d6278793          	addi	a5,a5,-670 # 2450 <threading_system_time>
    16f6:	439c                	lw	a5,0(a5)
    16f8:	863e                	mv	a2,a5
    16fa:	85ba                	mv	a1,a4
    16fc:	00001517          	auipc	a0,0x1
    1700:	afc50513          	addi	a0,a0,-1284 # 21f8 <schedule_edf_cbs+0x5a2>
    1704:	fffff097          	auipc	ra,0xfffff
    1708:	45c080e7          	jalr	1116(ra) # b60 <printf>
            exit(0);
    170c:	4501                	li	a0,0
    170e:	fffff097          	auipc	ra,0xfffff
    1712:	f0c080e7          	jalr	-244(ra) # 61a <exit>
        }

    if (current_thread->remaining_time <= 0) {
    1716:	fd843783          	ld	a5,-40(s0)
    171a:	4bfc                	lw	a5,84(a5)
    171c:	02f04063          	bgtz	a5,173c <switch_handler+0x132>
        if (current_thread->is_real_time)
    1720:	fd843783          	ld	a5,-40(s0)
    1724:	43bc                	lw	a5,64(a5)
    1726:	c791                	beqz	a5,1732 <switch_handler+0x128>
            __rt_finish_current();
    1728:	00000097          	auipc	ra,0x0
    172c:	e0a080e7          	jalr	-502(ra) # 1532 <__rt_finish_current>
    1730:	a881                	j	1780 <switch_handler+0x176>
        else
            __finish_current();
    1732:	00000097          	auipc	ra,0x0
    1736:	d3c080e7          	jalr	-708(ra) # 146e <__finish_current>
    173a:	a099                	j	1780 <switch_handler+0x176>
    } else {
        // move the current thread to the end of the run_queue
        struct list_head *to_remove = current;
    173c:	00001797          	auipc	a5,0x1
    1740:	d0c78793          	addi	a5,a5,-756 # 2448 <current>
    1744:	639c                	ld	a5,0(a5)
    1746:	fcf43823          	sd	a5,-48(s0)
        current = current->prev;
    174a:	00001797          	auipc	a5,0x1
    174e:	cfe78793          	addi	a5,a5,-770 # 2448 <current>
    1752:	639c                	ld	a5,0(a5)
    1754:	6798                	ld	a4,8(a5)
    1756:	00001797          	auipc	a5,0x1
    175a:	cf278793          	addi	a5,a5,-782 # 2448 <current>
    175e:	e398                	sd	a4,0(a5)
        list_del(to_remove);
    1760:	fd043503          	ld	a0,-48(s0)
    1764:	00000097          	auipc	ra,0x0
    1768:	840080e7          	jalr	-1984(ra) # fa4 <list_del>
        list_add_tail(to_remove, &run_queue);
    176c:	00001597          	auipc	a1,0x1
    1770:	c9458593          	addi	a1,a1,-876 # 2400 <run_queue>
    1774:	fd043503          	ld	a0,-48(s0)
    1778:	fffff097          	auipc	ra,0xfffff
    177c:	7d0080e7          	jalr	2000(ra) # f48 <list_add_tail>
    }

    __release();
    1780:	00000097          	auipc	ra,0x0
    1784:	ab0080e7          	jalr	-1360(ra) # 1230 <__release>
    __schedule();
    1788:	00000097          	auipc	ra,0x0
    178c:	1f8080e7          	jalr	504(ra) # 1980 <__schedule>
    __dispatch();
    1790:	00000097          	auipc	ra,0x0
    1794:	026080e7          	jalr	38(ra) # 17b6 <__dispatch>
    thrdresume(main_thrd_id);
    1798:	00001797          	auipc	a5,0x1
    179c:	c8878793          	addi	a5,a5,-888 # 2420 <main_thrd_id>
    17a0:	439c                	lw	a5,0(a5)
    17a2:	853e                	mv	a0,a5
    17a4:	fffff097          	auipc	ra,0xfffff
    17a8:	f1e080e7          	jalr	-226(ra) # 6c2 <thrdresume>
}
    17ac:	0001                	nop
    17ae:	70e2                	ld	ra,56(sp)
    17b0:	7442                	ld	s0,48(sp)
    17b2:	6121                	addi	sp,sp,64
    17b4:	8082                	ret

00000000000017b6 <__dispatch>:

void __dispatch()
{
    17b6:	7179                	addi	sp,sp,-48
    17b8:	f406                	sd	ra,40(sp)
    17ba:	f022                	sd	s0,32(sp)
    17bc:	1800                	addi	s0,sp,48
    if (current == &run_queue) {
    17be:	00001797          	auipc	a5,0x1
    17c2:	c8a78793          	addi	a5,a5,-886 # 2448 <current>
    17c6:	6398                	ld	a4,0(a5)
    17c8:	00001797          	auipc	a5,0x1
    17cc:	c3878793          	addi	a5,a5,-968 # 2400 <run_queue>
    17d0:	1af70363          	beq	a4,a5,1976 <__dispatch+0x1c0>
    if (allocated_time < 0) {
        fprintf(2, "[FATAL] allocated_time is negative\n");
        exit(1);
    }
    
    struct thread *current_thread = list_entry(current, struct thread, thread_list);
    17d4:	00001797          	auipc	a5,0x1
    17d8:	c7478793          	addi	a5,a5,-908 # 2448 <current>
    17dc:	639c                	ld	a5,0(a5)
    17de:	fef43423          	sd	a5,-24(s0)
    17e2:	fe843783          	ld	a5,-24(s0)
    17e6:	fd878793          	addi	a5,a5,-40
    17ea:	fef43023          	sd	a5,-32(s0)
    printf("    my print:dispatch thread#%d at %d: allocated_time=%d, remaining_time= %d \n", current_thread->ID, threading_system_time, allocated_time,current_thread->remaining_time);
    17ee:	fe043783          	ld	a5,-32(s0)
    17f2:	5fcc                	lw	a1,60(a5)
    17f4:	00001797          	auipc	a5,0x1
    17f8:	c5c78793          	addi	a5,a5,-932 # 2450 <threading_system_time>
    17fc:	4390                	lw	a2,0(a5)
    17fe:	00001797          	auipc	a5,0x1
    1802:	c5a78793          	addi	a5,a5,-934 # 2458 <allocated_time>
    1806:	6394                	ld	a3,0(a5)
    1808:	fe043783          	ld	a5,-32(s0)
    180c:	4bfc                	lw	a5,84(a5)
    180e:	873e                	mv	a4,a5
    1810:	00001517          	auipc	a0,0x1
    1814:	a1850513          	addi	a0,a0,-1512 # 2228 <schedule_edf_cbs+0x5d2>
    1818:	fffff097          	auipc	ra,0xfffff
    181c:	348080e7          	jalr	840(ra) # b60 <printf>

    if (current_thread->is_real_time && allocated_time == 0 && current_thread->cbs.is_hard_rt) {
    1820:	fe043783          	ld	a5,-32(s0)
    1824:	43bc                	lw	a5,64(a5)
    1826:	c3a1                	beqz	a5,1866 <__dispatch+0xb0>
    1828:	00001797          	auipc	a5,0x1
    182c:	c3078793          	addi	a5,a5,-976 # 2458 <allocated_time>
    1830:	639c                	ld	a5,0(a5)
    1832:	eb95                	bnez	a5,1866 <__dispatch+0xb0>
    1834:	fe043783          	ld	a5,-32(s0)
    1838:	57fc                	lw	a5,108(a5)
    183a:	c795                	beqz	a5,1866 <__dispatch+0xb0>
        printf("thread#%d misses a deadline at %d in dispatch\n", current_thread->ID, current_thread->current_deadline);
    183c:	fe043783          	ld	a5,-32(s0)
    1840:	5fd8                	lw	a4,60(a5)
    1842:	fe043783          	ld	a5,-32(s0)
    1846:	4fbc                	lw	a5,88(a5)
    1848:	863e                	mv	a2,a5
    184a:	85ba                	mv	a1,a4
    184c:	00001517          	auipc	a0,0x1
    1850:	a2c50513          	addi	a0,a0,-1492 # 2278 <schedule_edf_cbs+0x622>
    1854:	fffff097          	auipc	ra,0xfffff
    1858:	30c080e7          	jalr	780(ra) # b60 <printf>
        exit(0);
    185c:	4501                	li	a0,0
    185e:	fffff097          	auipc	ra,0xfffff
    1862:	dbc080e7          	jalr	-580(ra) # 61a <exit>
    }

    printf("dispatch thread#%d at %d: allocated_time=%d\n", current_thread->ID, threading_system_time, allocated_time);
    1866:	fe043783          	ld	a5,-32(s0)
    186a:	5fd8                	lw	a4,60(a5)
    186c:	00001797          	auipc	a5,0x1
    1870:	be478793          	addi	a5,a5,-1052 # 2450 <threading_system_time>
    1874:	4390                	lw	a2,0(a5)
    1876:	00001797          	auipc	a5,0x1
    187a:	be278793          	addi	a5,a5,-1054 # 2458 <allocated_time>
    187e:	639c                	ld	a5,0(a5)
    1880:	86be                	mv	a3,a5
    1882:	85ba                	mv	a1,a4
    1884:	00001517          	auipc	a0,0x1
    1888:	a2450513          	addi	a0,a0,-1500 # 22a8 <schedule_edf_cbs+0x652>
    188c:	fffff097          	auipc	ra,0xfffff
    1890:	2d4080e7          	jalr	724(ra) # b60 <printf>

    if (current_thread->buf_set) {
    1894:	fe043783          	ld	a5,-32(s0)
    1898:	539c                	lw	a5,32(a5)
    189a:	c7a1                	beqz	a5,18e2 <__dispatch+0x12c>
        thrdstop(allocated_time, &(current_thread->thrdstop_context_id), switch_handler, (void *)allocated_time);
    189c:	00001797          	auipc	a5,0x1
    18a0:	bbc78793          	addi	a5,a5,-1092 # 2458 <allocated_time>
    18a4:	639c                	ld	a5,0(a5)
    18a6:	0007871b          	sext.w	a4,a5
    18aa:	fe043783          	ld	a5,-32(s0)
    18ae:	03878593          	addi	a1,a5,56
    18b2:	00001797          	auipc	a5,0x1
    18b6:	ba678793          	addi	a5,a5,-1114 # 2458 <allocated_time>
    18ba:	639c                	ld	a5,0(a5)
    18bc:	86be                	mv	a3,a5
    18be:	00000617          	auipc	a2,0x0
    18c2:	d4c60613          	addi	a2,a2,-692 # 160a <switch_handler>
    18c6:	853a                	mv	a0,a4
    18c8:	fffff097          	auipc	ra,0xfffff
    18cc:	df2080e7          	jalr	-526(ra) # 6ba <thrdstop>
        thrdresume(current_thread->thrdstop_context_id);
    18d0:	fe043783          	ld	a5,-32(s0)
    18d4:	5f9c                	lw	a5,56(a5)
    18d6:	853e                	mv	a0,a5
    18d8:	fffff097          	auipc	ra,0xfffff
    18dc:	dea080e7          	jalr	-534(ra) # 6c2 <thrdresume>
    18e0:	a071                	j	196c <__dispatch+0x1b6>
    } else {
        current_thread->buf_set = 1;
    18e2:	fe043783          	ld	a5,-32(s0)
    18e6:	4705                	li	a4,1
    18e8:	d398                	sw	a4,32(a5)
        unsigned long new_stack_p = (unsigned long)current_thread->stack_p;
    18ea:	fe043783          	ld	a5,-32(s0)
    18ee:	6f9c                	ld	a5,24(a5)
    18f0:	fcf43c23          	sd	a5,-40(s0)
        current_thread->thrdstop_context_id = -1;
    18f4:	fe043783          	ld	a5,-32(s0)
    18f8:	577d                	li	a4,-1
    18fa:	df98                	sw	a4,56(a5)
        thrdstop(allocated_time, &(current_thread->thrdstop_context_id), switch_handler, (void *)allocated_time);
    18fc:	00001797          	auipc	a5,0x1
    1900:	b5c78793          	addi	a5,a5,-1188 # 2458 <allocated_time>
    1904:	639c                	ld	a5,0(a5)
    1906:	0007871b          	sext.w	a4,a5
    190a:	fe043783          	ld	a5,-32(s0)
    190e:	03878593          	addi	a1,a5,56
    1912:	00001797          	auipc	a5,0x1
    1916:	b4678793          	addi	a5,a5,-1210 # 2458 <allocated_time>
    191a:	639c                	ld	a5,0(a5)
    191c:	86be                	mv	a3,a5
    191e:	00000617          	auipc	a2,0x0
    1922:	cec60613          	addi	a2,a2,-788 # 160a <switch_handler>
    1926:	853a                	mv	a0,a4
    1928:	fffff097          	auipc	ra,0xfffff
    192c:	d92080e7          	jalr	-622(ra) # 6ba <thrdstop>
        if (current_thread->thrdstop_context_id < 0) {
    1930:	fe043783          	ld	a5,-32(s0)
    1934:	5f9c                	lw	a5,56(a5)
    1936:	0207d063          	bgez	a5,1956 <__dispatch+0x1a0>
            fprintf(2, "[ERROR] number of threads may exceed MAX_THRD_NUM\n");
    193a:	00001597          	auipc	a1,0x1
    193e:	99e58593          	addi	a1,a1,-1634 # 22d8 <schedule_edf_cbs+0x682>
    1942:	4509                	li	a0,2
    1944:	fffff097          	auipc	ra,0xfffff
    1948:	1c4080e7          	jalr	452(ra) # b08 <fprintf>
            exit(1);
    194c:	4505                	li	a0,1
    194e:	fffff097          	auipc	ra,0xfffff
    1952:	ccc080e7          	jalr	-820(ra) # 61a <exit>
        }

        // set sp to stack pointer of current thread.
        asm volatile("mv sp, %0"
    1956:	fd843783          	ld	a5,-40(s0)
    195a:	813e                	mv	sp,a5
                     :
                     : "r"(new_stack_p));
        current_thread->fp(current_thread->arg);
    195c:	fe043783          	ld	a5,-32(s0)
    1960:	6398                	ld	a4,0(a5)
    1962:	fe043783          	ld	a5,-32(s0)
    1966:	679c                	ld	a5,8(a5)
    1968:	853e                	mv	a0,a5
    196a:	9702                	jalr	a4
    }
    thread_exit();
    196c:	00000097          	auipc	ra,0x0
    1970:	a5a080e7          	jalr	-1446(ra) # 13c6 <thread_exit>
    1974:	a011                	j	1978 <__dispatch+0x1c2>
        return;
    1976:	0001                	nop
}
    1978:	70a2                	ld	ra,40(sp)
    197a:	7402                	ld	s0,32(sp)
    197c:	6145                	addi	sp,sp,48
    197e:	8082                	ret

0000000000001980 <__schedule>:

void __schedule()
{
    1980:	711d                	addi	sp,sp,-96
    1982:	ec86                	sd	ra,88(sp)
    1984:	e8a2                	sd	s0,80(sp)
    1986:	1080                	addi	s0,sp,96
    struct threads_sched_args args = {
    1988:	00001797          	auipc	a5,0x1
    198c:	ac878793          	addi	a5,a5,-1336 # 2450 <threading_system_time>
    1990:	439c                	lw	a5,0(a5)
    1992:	fcf42c23          	sw	a5,-40(s0)
    1996:	4789                	li	a5,2
    1998:	fcf42e23          	sw	a5,-36(s0)
    199c:	00001797          	auipc	a5,0x1
    19a0:	a6478793          	addi	a5,a5,-1436 # 2400 <run_queue>
    19a4:	fef43023          	sd	a5,-32(s0)
    19a8:	00001797          	auipc	a5,0x1
    19ac:	a6878793          	addi	a5,a5,-1432 # 2410 <release_queue>
    19b0:	fef43423          	sd	a5,-24(s0)
#ifdef THREAD_SCHEDULER_PRIORITY_RR
    r = schedule_priority_rr(args);
#endif

#ifdef THREAD_SCHEDULER_EDF_CBS
    r = schedule_edf_cbs(args);
    19b4:	fd843783          	ld	a5,-40(s0)
    19b8:	faf43023          	sd	a5,-96(s0)
    19bc:	fe043783          	ld	a5,-32(s0)
    19c0:	faf43423          	sd	a5,-88(s0)
    19c4:	fe843783          	ld	a5,-24(s0)
    19c8:	faf43823          	sd	a5,-80(s0)
    19cc:	fa040793          	addi	a5,s0,-96
    19d0:	853e                	mv	a0,a5
    19d2:	00000097          	auipc	ra,0x0
    19d6:	284080e7          	jalr	644(ra) # 1c56 <schedule_edf_cbs>
    19da:	872a                	mv	a4,a0
    19dc:	87ae                	mv	a5,a1
    19de:	fce43423          	sd	a4,-56(s0)
    19e2:	fcf43823          	sd	a5,-48(s0)
//     r = schedule_edf_cbs(args);
// #else
//     r = schedule_default(args);
// #endif

    current = r.scheduled_thread_list_member;
    19e6:	fc843703          	ld	a4,-56(s0)
    19ea:	00001797          	auipc	a5,0x1
    19ee:	a5e78793          	addi	a5,a5,-1442 # 2448 <current>
    19f2:	e398                	sd	a4,0(a5)
    allocated_time = r.allocated_time;
    19f4:	fd042783          	lw	a5,-48(s0)
    19f8:	873e                	mv	a4,a5
    19fa:	00001797          	auipc	a5,0x1
    19fe:	a5e78793          	addi	a5,a5,-1442 # 2458 <allocated_time>
    1a02:	e398                	sd	a4,0(a5)
}
    1a04:	0001                	nop
    1a06:	60e6                	ld	ra,88(sp)
    1a08:	6446                	ld	s0,80(sp)
    1a0a:	6125                	addi	sp,sp,96
    1a0c:	8082                	ret

0000000000001a0e <back_to_main_handler>:

void back_to_main_handler(void *arg)
{
    1a0e:	1101                	addi	sp,sp,-32
    1a10:	ec06                	sd	ra,24(sp)
    1a12:	e822                	sd	s0,16(sp)
    1a14:	1000                	addi	s0,sp,32
    1a16:	fea43423          	sd	a0,-24(s0)
    sleeping = 0;
    1a1a:	00001797          	auipc	a5,0x1
    1a1e:	a3a78793          	addi	a5,a5,-1478 # 2454 <sleeping>
    1a22:	0007a023          	sw	zero,0(a5)
    threading_system_time += (uint64)arg;
    1a26:	fe843783          	ld	a5,-24(s0)
    1a2a:	0007871b          	sext.w	a4,a5
    1a2e:	00001797          	auipc	a5,0x1
    1a32:	a2278793          	addi	a5,a5,-1502 # 2450 <threading_system_time>
    1a36:	439c                	lw	a5,0(a5)
    1a38:	2781                	sext.w	a5,a5
    1a3a:	9fb9                	addw	a5,a5,a4
    1a3c:	2781                	sext.w	a5,a5
    1a3e:	0007871b          	sext.w	a4,a5
    1a42:	00001797          	auipc	a5,0x1
    1a46:	a0e78793          	addi	a5,a5,-1522 # 2450 <threading_system_time>
    1a4a:	c398                	sw	a4,0(a5)
    thrdresume(main_thrd_id);
    1a4c:	00001797          	auipc	a5,0x1
    1a50:	9d478793          	addi	a5,a5,-1580 # 2420 <main_thrd_id>
    1a54:	439c                	lw	a5,0(a5)
    1a56:	853e                	mv	a0,a5
    1a58:	fffff097          	auipc	ra,0xfffff
    1a5c:	c6a080e7          	jalr	-918(ra) # 6c2 <thrdresume>
}
    1a60:	0001                	nop
    1a62:	60e2                	ld	ra,24(sp)
    1a64:	6442                	ld	s0,16(sp)
    1a66:	6105                	addi	sp,sp,32
    1a68:	8082                	ret

0000000000001a6a <thread_start_threading>:

void thread_start_threading()
{
    1a6a:	1141                	addi	sp,sp,-16
    1a6c:	e406                	sd	ra,8(sp)
    1a6e:	e022                	sd	s0,0(sp)
    1a70:	0800                	addi	s0,sp,16
    threading_system_time = 0;
    1a72:	00001797          	auipc	a5,0x1
    1a76:	9de78793          	addi	a5,a5,-1570 # 2450 <threading_system_time>
    1a7a:	0007a023          	sw	zero,0(a5)
    current = &run_queue;
    1a7e:	00001797          	auipc	a5,0x1
    1a82:	9ca78793          	addi	a5,a5,-1590 # 2448 <current>
    1a86:	00001717          	auipc	a4,0x1
    1a8a:	97a70713          	addi	a4,a4,-1670 # 2400 <run_queue>
    1a8e:	e398                	sd	a4,0(a5)

    // call thrdstop just for obtain an ID
    thrdstop(1000, &main_thrd_id, back_to_main_handler, (void *)0);
    1a90:	4681                	li	a3,0
    1a92:	00000617          	auipc	a2,0x0
    1a96:	f7c60613          	addi	a2,a2,-132 # 1a0e <back_to_main_handler>
    1a9a:	00001597          	auipc	a1,0x1
    1a9e:	98658593          	addi	a1,a1,-1658 # 2420 <main_thrd_id>
    1aa2:	3e800513          	li	a0,1000
    1aa6:	fffff097          	auipc	ra,0xfffff
    1aaa:	c14080e7          	jalr	-1004(ra) # 6ba <thrdstop>
    cancelthrdstop(main_thrd_id, 0);
    1aae:	00001797          	auipc	a5,0x1
    1ab2:	97278793          	addi	a5,a5,-1678 # 2420 <main_thrd_id>
    1ab6:	439c                	lw	a5,0(a5)
    1ab8:	4581                	li	a1,0
    1aba:	853e                	mv	a0,a5
    1abc:	fffff097          	auipc	ra,0xfffff
    1ac0:	c0e080e7          	jalr	-1010(ra) # 6ca <cancelthrdstop>

    while (!list_empty(&run_queue) || !list_empty(&release_queue)) {
    1ac4:	a0c9                	j	1b86 <thread_start_threading+0x11c>
        __release();
    1ac6:	fffff097          	auipc	ra,0xfffff
    1aca:	76a080e7          	jalr	1898(ra) # 1230 <__release>
        __schedule();
    1ace:	00000097          	auipc	ra,0x0
    1ad2:	eb2080e7          	jalr	-334(ra) # 1980 <__schedule>
        cancelthrdstop(main_thrd_id, 0);
    1ad6:	00001797          	auipc	a5,0x1
    1ada:	94a78793          	addi	a5,a5,-1718 # 2420 <main_thrd_id>
    1ade:	439c                	lw	a5,0(a5)
    1ae0:	4581                	li	a1,0
    1ae2:	853e                	mv	a0,a5
    1ae4:	fffff097          	auipc	ra,0xfffff
    1ae8:	be6080e7          	jalr	-1050(ra) # 6ca <cancelthrdstop>
        __dispatch();
    1aec:	00000097          	auipc	ra,0x0
    1af0:	cca080e7          	jalr	-822(ra) # 17b6 <__dispatch>

        if (list_empty(&run_queue) && list_empty(&release_queue)) {
    1af4:	00001517          	auipc	a0,0x1
    1af8:	90c50513          	addi	a0,a0,-1780 # 2400 <run_queue>
    1afc:	fffff097          	auipc	ra,0xfffff
    1b00:	4f2080e7          	jalr	1266(ra) # fee <list_empty>
    1b04:	87aa                	mv	a5,a0
    1b06:	cb99                	beqz	a5,1b1c <thread_start_threading+0xb2>
    1b08:	00001517          	auipc	a0,0x1
    1b0c:	90850513          	addi	a0,a0,-1784 # 2410 <release_queue>
    1b10:	fffff097          	auipc	ra,0xfffff
    1b14:	4de080e7          	jalr	1246(ra) # fee <list_empty>
    1b18:	87aa                	mv	a5,a0
    1b1a:	ebd9                	bnez	a5,1bb0 <thread_start_threading+0x146>
            break;
        }

        // no thread in run_queue, release_queue not empty
        printf("run_queue is empty, sleep for %d ticks\n", allocated_time);
    1b1c:	00001797          	auipc	a5,0x1
    1b20:	93c78793          	addi	a5,a5,-1732 # 2458 <allocated_time>
    1b24:	639c                	ld	a5,0(a5)
    1b26:	85be                	mv	a1,a5
    1b28:	00000517          	auipc	a0,0x0
    1b2c:	7e850513          	addi	a0,a0,2024 # 2310 <schedule_edf_cbs+0x6ba>
    1b30:	fffff097          	auipc	ra,0xfffff
    1b34:	030080e7          	jalr	48(ra) # b60 <printf>
        sleeping = 1;
    1b38:	00001797          	auipc	a5,0x1
    1b3c:	91c78793          	addi	a5,a5,-1764 # 2454 <sleeping>
    1b40:	4705                	li	a4,1
    1b42:	c398                	sw	a4,0(a5)
        thrdstop(allocated_time, &main_thrd_id, back_to_main_handler, (void *)allocated_time);
    1b44:	00001797          	auipc	a5,0x1
    1b48:	91478793          	addi	a5,a5,-1772 # 2458 <allocated_time>
    1b4c:	639c                	ld	a5,0(a5)
    1b4e:	0007871b          	sext.w	a4,a5
    1b52:	00001797          	auipc	a5,0x1
    1b56:	90678793          	addi	a5,a5,-1786 # 2458 <allocated_time>
    1b5a:	639c                	ld	a5,0(a5)
    1b5c:	86be                	mv	a3,a5
    1b5e:	00000617          	auipc	a2,0x0
    1b62:	eb060613          	addi	a2,a2,-336 # 1a0e <back_to_main_handler>
    1b66:	00001597          	auipc	a1,0x1
    1b6a:	8ba58593          	addi	a1,a1,-1862 # 2420 <main_thrd_id>
    1b6e:	853a                	mv	a0,a4
    1b70:	fffff097          	auipc	ra,0xfffff
    1b74:	b4a080e7          	jalr	-1206(ra) # 6ba <thrdstop>
        while (sleeping) {
    1b78:	0001                	nop
    1b7a:	00001797          	auipc	a5,0x1
    1b7e:	8da78793          	addi	a5,a5,-1830 # 2454 <sleeping>
    1b82:	439c                	lw	a5,0(a5)
    1b84:	fbfd                	bnez	a5,1b7a <thread_start_threading+0x110>
    while (!list_empty(&run_queue) || !list_empty(&release_queue)) {
    1b86:	00001517          	auipc	a0,0x1
    1b8a:	87a50513          	addi	a0,a0,-1926 # 2400 <run_queue>
    1b8e:	fffff097          	auipc	ra,0xfffff
    1b92:	460080e7          	jalr	1120(ra) # fee <list_empty>
    1b96:	87aa                	mv	a5,a0
    1b98:	d79d                	beqz	a5,1ac6 <thread_start_threading+0x5c>
    1b9a:	00001517          	auipc	a0,0x1
    1b9e:	87650513          	addi	a0,a0,-1930 # 2410 <release_queue>
    1ba2:	fffff097          	auipc	ra,0xfffff
    1ba6:	44c080e7          	jalr	1100(ra) # fee <list_empty>
    1baa:	87aa                	mv	a5,a0
    1bac:	df89                	beqz	a5,1ac6 <thread_start_threading+0x5c>
            // zzz...
        }
    }
}
    1bae:	a011                	j	1bb2 <thread_start_threading+0x148>
            break;
    1bb0:	0001                	nop
}
    1bb2:	0001                	nop
    1bb4:	60a2                	ld	ra,8(sp)
    1bb6:	6402                	ld	s0,0(sp)
    1bb8:	0141                	addi	sp,sp,16
    1bba:	8082                	ret

0000000000001bbc <__check_deadline_miss>:

/* MP3 Part 2 - Real-Time Scheduling*/

#if defined(THREAD_SCHEDULER_EDF_CBS) || defined(THREAD_SCHEDULER_DM)
static struct thread *__check_deadline_miss(struct list_head *run_queue, int current_time)
{   
    1bbc:	7139                	addi	sp,sp,-64
    1bbe:	fc22                	sd	s0,56(sp)
    1bc0:	0080                	addi	s0,sp,64
    1bc2:	fca43423          	sd	a0,-56(s0)
    1bc6:	87ae                	mv	a5,a1
    1bc8:	fcf42223          	sw	a5,-60(s0)
    
    struct thread *th = NULL;
    1bcc:	fe043423          	sd	zero,-24(s0)
    struct thread *thread_missing_deadline = NULL;
    1bd0:	fe043023          	sd	zero,-32(s0)
    list_for_each_entry(th, run_queue, thread_list) {
    1bd4:	fc843783          	ld	a5,-56(s0)
    1bd8:	639c                	ld	a5,0(a5)
    1bda:	fcf43c23          	sd	a5,-40(s0)
    1bde:	fd843783          	ld	a5,-40(s0)
    1be2:	fd878793          	addi	a5,a5,-40
    1be6:	fef43423          	sd	a5,-24(s0)
    1bea:	a881                	j	1c3a <__check_deadline_miss+0x7e>
       // printf("    checking thread %d, its current deadline at time %d is %d\n", th->ID,current_time, th->current_deadline);
        if (th->current_deadline <= current_time) {
    1bec:	fe843783          	ld	a5,-24(s0)
    1bf0:	4fb8                	lw	a4,88(a5)
    1bf2:	fc442783          	lw	a5,-60(s0)
    1bf6:	2781                	sext.w	a5,a5
    1bf8:	02e7c663          	blt	a5,a4,1c24 <__check_deadline_miss+0x68>
            if (thread_missing_deadline == NULL)
    1bfc:	fe043783          	ld	a5,-32(s0)
    1c00:	e791                	bnez	a5,1c0c <__check_deadline_miss+0x50>
                thread_missing_deadline = th;
    1c02:	fe843783          	ld	a5,-24(s0)
    1c06:	fef43023          	sd	a5,-32(s0)
    1c0a:	a829                	j	1c24 <__check_deadline_miss+0x68>
            else if (th->ID < thread_missing_deadline->ID)
    1c0c:	fe843783          	ld	a5,-24(s0)
    1c10:	5fd8                	lw	a4,60(a5)
    1c12:	fe043783          	ld	a5,-32(s0)
    1c16:	5fdc                	lw	a5,60(a5)
    1c18:	00f75663          	bge	a4,a5,1c24 <__check_deadline_miss+0x68>
                thread_missing_deadline = th;
    1c1c:	fe843783          	ld	a5,-24(s0)
    1c20:	fef43023          	sd	a5,-32(s0)
    list_for_each_entry(th, run_queue, thread_list) {
    1c24:	fe843783          	ld	a5,-24(s0)
    1c28:	779c                	ld	a5,40(a5)
    1c2a:	fcf43823          	sd	a5,-48(s0)
    1c2e:	fd043783          	ld	a5,-48(s0)
    1c32:	fd878793          	addi	a5,a5,-40
    1c36:	fef43423          	sd	a5,-24(s0)
    1c3a:	fe843783          	ld	a5,-24(s0)
    1c3e:	02878793          	addi	a5,a5,40
    1c42:	fc843703          	ld	a4,-56(s0)
    1c46:	faf713e3          	bne	a4,a5,1bec <__check_deadline_miss+0x30>
        }
    }
    return thread_missing_deadline;
    1c4a:	fe043783          	ld	a5,-32(s0)
}
    1c4e:	853e                	mv	a0,a5
    1c50:	7462                	ld	s0,56(sp)
    1c52:	6121                	addi	sp,sp,64
    1c54:	8082                	ret

0000000000001c56 <schedule_edf_cbs>:
{
    // TO DO
}*/
//  EDF_CBS scheduler
struct threads_sched_result schedule_edf_cbs(struct threads_sched_args args)
{
    1c56:	7131                	addi	sp,sp,-192
    1c58:	fd06                	sd	ra,184(sp)
    1c5a:	f922                	sd	s0,176(sp)
    1c5c:	f526                	sd	s1,168(sp)
    1c5e:	f14a                	sd	s2,160(sp)
    1c60:	ed4e                	sd	s3,152(sp)
    1c62:	0180                	addi	s0,sp,192
    1c64:	84aa                	mv	s1,a0
   
   
    struct threads_sched_result r;
    struct thread *next = NULL;
    1c66:	fa043023          	sd	zero,-96(s0)
    struct release_queue_entry *awating_rqe = NULL;
    1c6a:	fa043423          	sd	zero,-88(s0)
    struct release_queue_entry *h_awating_rqe = NULL;
    1c6e:	fa043823          	sd	zero,-80(s0)
    struct thread *th = NULL;
    1c72:	fa043c23          	sd	zero,-72(s0)

    
    struct thread *thread_missing_deadline = __check_deadline_miss(args.run_queue,args.current_time);
    1c76:	649c                	ld	a5,8(s1)
    1c78:	4098                	lw	a4,0(s1)
    1c7a:	85ba                	mv	a1,a4
    1c7c:	853e                	mv	a0,a5
    1c7e:	00000097          	auipc	ra,0x0
    1c82:	f3e080e7          	jalr	-194(ra) # 1bbc <__check_deadline_miss>
    1c86:	f8a43c23          	sd	a0,-104(s0)
    if(thread_missing_deadline){
    1c8a:	f9843783          	ld	a5,-104(s0)
    1c8e:	c7b5                	beqz	a5,1cfa <schedule_edf_cbs+0xa4>
        printf("     we have found a missing thread and its thread %d \n", thread_missing_deadline->ID);
    1c90:	f9843783          	ld	a5,-104(s0)
    1c94:	5fdc                	lw	a5,60(a5)
    1c96:	85be                	mv	a1,a5
    1c98:	00000517          	auipc	a0,0x0
    1c9c:	6a050513          	addi	a0,a0,1696 # 2338 <schedule_edf_cbs+0x6e2>
    1ca0:	fffff097          	auipc	ra,0xfffff
    1ca4:	ec0080e7          	jalr	-320(ra) # b60 <printf>
        if(thread_missing_deadline->cbs.is_hard_rt){
    1ca8:	f9843783          	ld	a5,-104(s0)
    1cac:	57fc                	lw	a5,108(a5)
    1cae:	c395                	beqz	a5,1cd2 <schedule_edf_cbs+0x7c>
            r.scheduled_thread_list_member = &thread_missing_deadline->thread_list;
    1cb0:	f9843783          	ld	a5,-104(s0)
    1cb4:	02878793          	addi	a5,a5,40
    1cb8:	f4f43023          	sd	a5,-192(s0)
            r.allocated_time = 0;     
    1cbc:	f4042423          	sw	zero,-184(s0)
            return r;
    1cc0:	f4043783          	ld	a5,-192(s0)
    1cc4:	f4f43823          	sd	a5,-176(s0)
    1cc8:	f4843783          	ld	a5,-184(s0)
    1ccc:	f4f43c23          	sd	a5,-168(s0)
    1cd0:	a13d                	j	20fe <schedule_edf_cbs+0x4a8>
        }else{
            thread_missing_deadline->current_deadline = args.current_time + thread_missing_deadline->period;
    1cd2:	4098                	lw	a4,0(s1)
    1cd4:	f9843783          	ld	a5,-104(s0)
    1cd8:	47fc                	lw	a5,76(a5)
    1cda:	9fb9                	addw	a5,a5,a4
    1cdc:	0007871b          	sext.w	a4,a5
    1ce0:	f9843783          	ld	a5,-104(s0)
    1ce4:	cfb8                	sw	a4,88(a5)
            thread_missing_deadline->cbs.remaining_budget = thread_missing_deadline->cbs.budget;
    1ce6:	f9843783          	ld	a5,-104(s0)
    1cea:	53f8                	lw	a4,100(a5)
    1cec:	f9843783          	ld	a5,-104(s0)
    1cf0:	d7b8                	sw	a4,104(a5)
            thread_missing_deadline->cbs.is_throttled=0;
    1cf2:	f9843783          	ld	a5,-104(s0)
    1cf6:	0607a823          	sw	zero,112(a5)
        else if (th->cbs.is_hard_rt && th->current_deadline == next->current_deadline && th->ID < next->ID)
            next = th;
    }
*/
    // if no hard th, find soft th
    if(!next){
    1cfa:	fa043783          	ld	a5,-96(s0)
    1cfe:	e7dd                	bnez	a5,1dac <schedule_edf_cbs+0x156>
        list_for_each_entry(th, args.run_queue, thread_list) {
    1d00:	649c                	ld	a5,8(s1)
    1d02:	639c                	ld	a5,0(a5)
    1d04:	f8f43823          	sd	a5,-112(s0)
    1d08:	f9043783          	ld	a5,-112(s0)
    1d0c:	fd878793          	addi	a5,a5,-40
    1d10:	faf43c23          	sd	a5,-72(s0)
    1d14:	a069                	j	1d9e <schedule_edf_cbs+0x148>
            printf("     we're looking at thread %d and is_throt is %d\n", th->ID, th->cbs.is_throttled);
    1d16:	fb843783          	ld	a5,-72(s0)
    1d1a:	5fd8                	lw	a4,60(a5)
    1d1c:	fb843783          	ld	a5,-72(s0)
    1d20:	5bbc                	lw	a5,112(a5)
    1d22:	863e                	mv	a2,a5
    1d24:	85ba                	mv	a1,a4
    1d26:	00000517          	auipc	a0,0x0
    1d2a:	64a50513          	addi	a0,a0,1610 # 2370 <schedule_edf_cbs+0x71a>
    1d2e:	fffff097          	auipc	ra,0xfffff
    1d32:	e32080e7          	jalr	-462(ra) # b60 <printf>
            if (next == NULL)
    1d36:	fa043783          	ld	a5,-96(s0)
    1d3a:	e791                	bnez	a5,1d46 <schedule_edf_cbs+0xf0>
                next = th;
    1d3c:	fb843783          	ld	a5,-72(s0)
    1d40:	faf43023          	sd	a5,-96(s0)
    1d44:	a091                	j	1d88 <schedule_edf_cbs+0x132>
            else if(th->current_deadline < next->current_deadline)
    1d46:	fb843783          	ld	a5,-72(s0)
    1d4a:	4fb8                	lw	a4,88(a5)
    1d4c:	fa043783          	ld	a5,-96(s0)
    1d50:	4fbc                	lw	a5,88(a5)
    1d52:	00f75763          	bge	a4,a5,1d60 <schedule_edf_cbs+0x10a>
                next = th;
    1d56:	fb843783          	ld	a5,-72(s0)
    1d5a:	faf43023          	sd	a5,-96(s0)
    1d5e:	a02d                	j	1d88 <schedule_edf_cbs+0x132>
            else if (th->current_deadline == next->current_deadline && th->ID < next->ID)
    1d60:	fb843783          	ld	a5,-72(s0)
    1d64:	4fb8                	lw	a4,88(a5)
    1d66:	fa043783          	ld	a5,-96(s0)
    1d6a:	4fbc                	lw	a5,88(a5)
    1d6c:	00f71e63          	bne	a4,a5,1d88 <schedule_edf_cbs+0x132>
    1d70:	fb843783          	ld	a5,-72(s0)
    1d74:	5fd8                	lw	a4,60(a5)
    1d76:	fa043783          	ld	a5,-96(s0)
    1d7a:	5fdc                	lw	a5,60(a5)
    1d7c:	00f75663          	bge	a4,a5,1d88 <schedule_edf_cbs+0x132>
                next = th;
    1d80:	fb843783          	ld	a5,-72(s0)
    1d84:	faf43023          	sd	a5,-96(s0)
        list_for_each_entry(th, args.run_queue, thread_list) {
    1d88:	fb843783          	ld	a5,-72(s0)
    1d8c:	779c                	ld	a5,40(a5)
    1d8e:	f8f43423          	sd	a5,-120(s0)
    1d92:	f8843783          	ld	a5,-120(s0)
    1d96:	fd878793          	addi	a5,a5,-40
    1d9a:	faf43c23          	sd	a5,-72(s0)
    1d9e:	fb843783          	ld	a5,-72(s0)
    1da2:	02878713          	addi	a4,a5,40
    1da6:	649c                	ld	a5,8(s1)
    1da8:	f6f717e3          	bne	a4,a5,1d16 <schedule_edf_cbs+0xc0>
        }
    }
    printf("    next thread will be no.%d\n", next->ID);
    1dac:	fa043783          	ld	a5,-96(s0)
    1db0:	5fdc                	lw	a5,60(a5)
    1db2:	85be                	mv	a1,a5
    1db4:	00000517          	auipc	a0,0x0
    1db8:	5f450513          	addi	a0,a0,1524 # 23a8 <schedule_edf_cbs+0x752>
    1dbc:	fffff097          	auipc	ra,0xfffff
    1dc0:	da4080e7          	jalr	-604(ra) # b60 <printf>

    if(next && next->cbs.is_throttled){
    1dc4:	fa043783          	ld	a5,-96(s0)
    1dc8:	cb95                	beqz	a5,1dfc <schedule_edf_cbs+0x1a6>
    1dca:	fa043783          	ld	a5,-96(s0)
    1dce:	5bbc                	lw	a5,112(a5)
    1dd0:	c795                	beqz	a5,1dfc <schedule_edf_cbs+0x1a6>
        // return throttled thread to release queue

        next->remaining_time=0;
    1dd2:	fa043783          	ld	a5,-96(s0)
    1dd6:	0407aa23          	sw	zero,84(a5)

        //__rt_finish_current();
        r.scheduled_thread_list_member = &next->thread_list;
    1dda:	fa043783          	ld	a5,-96(s0)
    1dde:	02878793          	addi	a5,a5,40
    1de2:	f4f43023          	sd	a5,-192(s0)
        r.allocated_time = 0;     
    1de6:	f4042423          	sw	zero,-184(s0)
        return r;
    1dea:	f4043783          	ld	a5,-192(s0)
    1dee:	f4f43823          	sd	a5,-176(s0)
    1df2:	f4843783          	ld	a5,-184(s0)
    1df6:	f4f43c23          	sd	a5,-168(s0)
    1dfa:	a611                	j	20fe <schedule_edf_cbs+0x4a8>
        } 
    }      
    


    struct release_queue_entry *ath=NULL;
    1dfc:	fc043023          	sd	zero,-64(s0)
    list_for_each_entry(ath, args.release_queue, thread_list) {// find thread in release queue witheralist dealine
    1e00:	689c                	ld	a5,16(s1)
    1e02:	639c                	ld	a5,0(a5)
    1e04:	f8f43023          	sd	a5,-128(s0)
    1e08:	f8043783          	ld	a5,-128(s0)
    1e0c:	17e1                	addi	a5,a5,-8
    1e0e:	fcf43023          	sd	a5,-64(s0)
    1e12:	a0b5                	j	1e7e <schedule_edf_cbs+0x228>
        if (awating_rqe == NULL) 
    1e14:	fa843783          	ld	a5,-88(s0)
    1e18:	e791                	bnez	a5,1e24 <schedule_edf_cbs+0x1ce>
            awating_rqe = ath;  
    1e1a:	fc043783          	ld	a5,-64(s0)
    1e1e:	faf43423          	sd	a5,-88(s0)
    1e22:	a0a1                	j	1e6a <schedule_edf_cbs+0x214>
        else if(awating_rqe->release_time > ath->release_time)
    1e24:	fa843783          	ld	a5,-88(s0)
    1e28:	4f98                	lw	a4,24(a5)
    1e2a:	fc043783          	ld	a5,-64(s0)
    1e2e:	4f9c                	lw	a5,24(a5)
    1e30:	00e7d763          	bge	a5,a4,1e3e <schedule_edf_cbs+0x1e8>
            awating_rqe = ath;
    1e34:	fc043783          	ld	a5,-64(s0)
    1e38:	faf43423          	sd	a5,-88(s0)
    1e3c:	a03d                	j	1e6a <schedule_edf_cbs+0x214>
        else if(awating_rqe->release_time == ath->release_time &&
    1e3e:	fa843783          	ld	a5,-88(s0)
    1e42:	4f98                	lw	a4,24(a5)
    1e44:	fc043783          	ld	a5,-64(s0)
    1e48:	4f9c                	lw	a5,24(a5)
    1e4a:	02f71063          	bne	a4,a5,1e6a <schedule_edf_cbs+0x214>
                awating_rqe->thrd->current_deadline > ath->thrd->current_deadline) 
    1e4e:	fa843783          	ld	a5,-88(s0)
    1e52:	639c                	ld	a5,0(a5)
    1e54:	4fb8                	lw	a4,88(a5)
    1e56:	fc043783          	ld	a5,-64(s0)
    1e5a:	639c                	ld	a5,0(a5)
    1e5c:	4fbc                	lw	a5,88(a5)
        else if(awating_rqe->release_time == ath->release_time &&
    1e5e:	00e7d663          	bge	a5,a4,1e6a <schedule_edf_cbs+0x214>
            awating_rqe = ath;  
    1e62:	fc043783          	ld	a5,-64(s0)
    1e66:	faf43423          	sd	a5,-88(s0)
    list_for_each_entry(ath, args.release_queue, thread_list) {// find thread in release queue witheralist dealine
    1e6a:	fc043783          	ld	a5,-64(s0)
    1e6e:	679c                	ld	a5,8(a5)
    1e70:	f6f43023          	sd	a5,-160(s0)
    1e74:	f6043783          	ld	a5,-160(s0)
    1e78:	17e1                	addi	a5,a5,-8
    1e7a:	fcf43023          	sd	a5,-64(s0)
    1e7e:	fc043783          	ld	a5,-64(s0)
    1e82:	00878713          	addi	a4,a5,8
    1e86:	689c                	ld	a5,16(s1)
    1e88:	f8f716e3          	bne	a4,a5,1e14 <schedule_edf_cbs+0x1be>
    }

    list_for_each_entry(ath, args.release_queue, thread_list) {// find hard thread in release queue with highest priority
    1e8c:	689c                	ld	a5,16(s1)
    1e8e:	639c                	ld	a5,0(a5)
    1e90:	f6f43c23          	sd	a5,-136(s0)
    1e94:	f7843783          	ld	a5,-136(s0)
    1e98:	17e1                	addi	a5,a5,-8
    1e9a:	fcf43023          	sd	a5,-64(s0)
    1e9e:	a069                	j	1f28 <schedule_edf_cbs+0x2d2>
        if (h_awating_rqe == NULL && ath->thrd->cbs.is_hard_rt) 
    1ea0:	fb043783          	ld	a5,-80(s0)
    1ea4:	eb99                	bnez	a5,1eba <schedule_edf_cbs+0x264>
    1ea6:	fc043783          	ld	a5,-64(s0)
    1eaa:	639c                	ld	a5,0(a5)
    1eac:	57fc                	lw	a5,108(a5)
    1eae:	c791                	beqz	a5,1eba <schedule_edf_cbs+0x264>
            h_awating_rqe = ath;  
    1eb0:	fc043783          	ld	a5,-64(s0)
    1eb4:	faf43823          	sd	a5,-80(s0)
    1eb8:	a8b1                	j	1f14 <schedule_edf_cbs+0x2be>
        else if(ath->thrd->cbs.is_hard_rt && 
    1eba:	fc043783          	ld	a5,-64(s0)
    1ebe:	639c                	ld	a5,0(a5)
    1ec0:	57fc                	lw	a5,108(a5)
    1ec2:	cf91                	beqz	a5,1ede <schedule_edf_cbs+0x288>
                h_awating_rqe->release_time > ath->release_time)
    1ec4:	fb043783          	ld	a5,-80(s0)
    1ec8:	4f98                	lw	a4,24(a5)
    1eca:	fc043783          	ld	a5,-64(s0)
    1ece:	4f9c                	lw	a5,24(a5)
        else if(ath->thrd->cbs.is_hard_rt && 
    1ed0:	00e7d763          	bge	a5,a4,1ede <schedule_edf_cbs+0x288>
            h_awating_rqe = ath;
    1ed4:	fc043783          	ld	a5,-64(s0)
    1ed8:	faf43823          	sd	a5,-80(s0)
    1edc:	a825                	j	1f14 <schedule_edf_cbs+0x2be>
        else if(ath->thrd->cbs.is_hard_rt && 
    1ede:	fc043783          	ld	a5,-64(s0)
    1ee2:	639c                	ld	a5,0(a5)
    1ee4:	57fc                	lw	a5,108(a5)
    1ee6:	c79d                	beqz	a5,1f14 <schedule_edf_cbs+0x2be>
                h_awating_rqe->release_time == ath->release_time &&
    1ee8:	fb043783          	ld	a5,-80(s0)
    1eec:	4f98                	lw	a4,24(a5)
    1eee:	fc043783          	ld	a5,-64(s0)
    1ef2:	4f9c                	lw	a5,24(a5)
        else if(ath->thrd->cbs.is_hard_rt && 
    1ef4:	02f71063          	bne	a4,a5,1f14 <schedule_edf_cbs+0x2be>
                h_awating_rqe->thrd->current_deadline > ath->thrd->current_deadline) 
    1ef8:	fb043783          	ld	a5,-80(s0)
    1efc:	639c                	ld	a5,0(a5)
    1efe:	4fb8                	lw	a4,88(a5)
    1f00:	fc043783          	ld	a5,-64(s0)
    1f04:	639c                	ld	a5,0(a5)
    1f06:	4fbc                	lw	a5,88(a5)
                h_awating_rqe->release_time == ath->release_time &&
    1f08:	00e7d663          	bge	a5,a4,1f14 <schedule_edf_cbs+0x2be>
                h_awating_rqe = ath;  
    1f0c:	fc043783          	ld	a5,-64(s0)
    1f10:	faf43823          	sd	a5,-80(s0)
    list_for_each_entry(ath, args.release_queue, thread_list) {// find hard thread in release queue with highest priority
    1f14:	fc043783          	ld	a5,-64(s0)
    1f18:	679c                	ld	a5,8(a5)
    1f1a:	f6f43423          	sd	a5,-152(s0)
    1f1e:	f6843783          	ld	a5,-152(s0)
    1f22:	17e1                	addi	a5,a5,-8
    1f24:	fcf43023          	sd	a5,-64(s0)
    1f28:	fc043783          	ld	a5,-64(s0)
    1f2c:	00878713          	addi	a4,a5,8
    1f30:	689c                	ld	a5,16(s1)
    1f32:	f6f717e3          	bne	a4,a5,1ea0 <schedule_edf_cbs+0x24a>
    }



    if(next && next->cbs.is_hard_rt)
    1f36:	fa043783          	ld	a5,-96(s0)
    1f3a:	cb89                	beqz	a5,1f4c <schedule_edf_cbs+0x2f6>
    1f3c:	fa043783          	ld	a5,-96(s0)
    1f40:	57fc                	lw	a5,108(a5)
    1f42:	c789                	beqz	a5,1f4c <schedule_edf_cbs+0x2f6>
        awating_rqe=h_awating_rqe;
    1f44:	fb043783          	ld	a5,-80(s0)
    1f48:	faf43423          	sd	a5,-88(s0)
   


    int time_untill_awaiting_thread = 0;
    1f4c:	fc042623          	sw	zero,-52(s0)
    int preempt_thread_flag=0;
    1f50:	fc042423          	sw	zero,-56(s0)
    if(awating_rqe){
    1f54:	fa843783          	ld	a5,-88(s0)
    1f58:	c3c5                	beqz	a5,1ff8 <schedule_edf_cbs+0x3a2>
        printf("    awating_rqe->thrd->ID is %d",awating_rqe->thrd->ID );
    1f5a:	fa843783          	ld	a5,-88(s0)
    1f5e:	639c                	ld	a5,0(a5)
    1f60:	5fdc                	lw	a5,60(a5)
    1f62:	85be                	mv	a1,a5
    1f64:	00000517          	auipc	a0,0x0
    1f68:	46450513          	addi	a0,a0,1124 # 23c8 <schedule_edf_cbs+0x772>
    1f6c:	fffff097          	auipc	ra,0xfffff
    1f70:	bf4080e7          	jalr	-1036(ra) # b60 <printf>
        time_untill_awaiting_thread=awating_rqe->release_time-args.current_time; 
    1f74:	fa843783          	ld	a5,-88(s0)
    1f78:	4f98                	lw	a4,24(a5)
    1f7a:	409c                	lw	a5,0(s1)
    1f7c:	40f707bb          	subw	a5,a4,a5
    1f80:	fcf42623          	sw	a5,-52(s0)
        if (next){
    1f84:	fa043783          	ld	a5,-96(s0)
    1f88:	cba5                	beqz	a5,1ff8 <schedule_edf_cbs+0x3a2>
            if(time_untill_awaiting_thread < next->remaining_time){
    1f8a:	fa043783          	ld	a5,-96(s0)
    1f8e:	4bf8                	lw	a4,84(a5)
    1f90:	fcc42783          	lw	a5,-52(s0)
    1f94:	2781                	sext.w	a5,a5
    1f96:	06e7d163          	bge	a5,a4,1ff8 <schedule_edf_cbs+0x3a2>
                if(!next->cbs.is_hard_rt && awating_rqe->thrd->cbs.is_hard_rt)
    1f9a:	fa043783          	ld	a5,-96(s0)
    1f9e:	57fc                	lw	a5,108(a5)
    1fa0:	eb91                	bnez	a5,1fb4 <schedule_edf_cbs+0x35e>
    1fa2:	fa843783          	ld	a5,-88(s0)
    1fa6:	639c                	ld	a5,0(a5)
    1fa8:	57fc                	lw	a5,108(a5)
    1faa:	c789                	beqz	a5,1fb4 <schedule_edf_cbs+0x35e>
                    preempt_thread_flag=1;    
    1fac:	4785                	li	a5,1
    1fae:	fcf42423          	sw	a5,-56(s0)
    1fb2:	a099                	j	1ff8 <schedule_edf_cbs+0x3a2>
                else if(next->current_deadline > awating_rqe->thrd->current_deadline)
    1fb4:	fa043783          	ld	a5,-96(s0)
    1fb8:	4fb8                	lw	a4,88(a5)
    1fba:	fa843783          	ld	a5,-88(s0)
    1fbe:	639c                	ld	a5,0(a5)
    1fc0:	4fbc                	lw	a5,88(a5)
    1fc2:	00e7d663          	bge	a5,a4,1fce <schedule_edf_cbs+0x378>
                    preempt_thread_flag=1;
    1fc6:	4785                	li	a5,1
    1fc8:	fcf42423          	sw	a5,-56(s0)
    1fcc:	a035                	j	1ff8 <schedule_edf_cbs+0x3a2>
                else if(next->current_deadline == awating_rqe->thrd->current_deadline && 
    1fce:	fa043783          	ld	a5,-96(s0)
    1fd2:	4fb8                	lw	a4,88(a5)
    1fd4:	fa843783          	ld	a5,-88(s0)
    1fd8:	639c                	ld	a5,0(a5)
    1fda:	4fbc                	lw	a5,88(a5)
    1fdc:	00f71e63          	bne	a4,a5,1ff8 <schedule_edf_cbs+0x3a2>
                        next->ID > awating_rqe->thrd->ID)
    1fe0:	fa043783          	ld	a5,-96(s0)
    1fe4:	5fd8                	lw	a4,60(a5)
    1fe6:	fa843783          	ld	a5,-88(s0)
    1fea:	639c                	ld	a5,0(a5)
    1fec:	5fdc                	lw	a5,60(a5)
                else if(next->current_deadline == awating_rqe->thrd->current_deadline && 
    1fee:	00e7d563          	bge	a5,a4,1ff8 <schedule_edf_cbs+0x3a2>
                    preempt_thread_flag=1;
    1ff2:	4785                	li	a5,1
    1ff4:	fcf42423          	sw	a5,-56(s0)

    
      

    //CBS mechanism
    if(next && !next->cbs.is_hard_rt){
    1ff8:	fa043783          	ld	a5,-96(s0)
    1ffc:	cbd1                	beqz	a5,2090 <schedule_edf_cbs+0x43a>
    1ffe:	fa043783          	ld	a5,-96(s0)
    2002:	57fc                	lw	a5,108(a5)
    2004:	e7d1                	bnez	a5,2090 <schedule_edf_cbs+0x43a>
        int time_until_deadline= next->current_deadline-args.current_time;
    2006:	fa043783          	ld	a5,-96(s0)
    200a:	4fb8                	lw	a4,88(a5)
    200c:	409c                	lw	a5,0(s1)
    200e:	40f707bb          	subw	a5,a4,a5
    2012:	f6f42a23          	sw	a5,-140(s0)

        if(next->period * next->cbs.remaining_budget > next->cbs.budget * time_until_deadline){
    2016:	fa043783          	ld	a5,-96(s0)
    201a:	47f8                	lw	a4,76(a5)
    201c:	fa043783          	ld	a5,-96(s0)
    2020:	57bc                	lw	a5,104(a5)
    2022:	02f707bb          	mulw	a5,a4,a5
    2026:	0007869b          	sext.w	a3,a5
    202a:	fa043783          	ld	a5,-96(s0)
    202e:	53fc                	lw	a5,100(a5)
    2030:	f7442703          	lw	a4,-140(s0)
    2034:	02f707bb          	mulw	a5,a4,a5
    2038:	2781                	sext.w	a5,a5
    203a:	8736                	mv	a4,a3
    203c:	02e7d263          	bge	a5,a4,2060 <schedule_edf_cbs+0x40a>
            next->current_deadline = args.current_time + next->period;
    2040:	4098                	lw	a4,0(s1)
    2042:	fa043783          	ld	a5,-96(s0)
    2046:	47fc                	lw	a5,76(a5)
    2048:	9fb9                	addw	a5,a5,a4
    204a:	0007871b          	sext.w	a4,a5
    204e:	fa043783          	ld	a5,-96(s0)
    2052:	cfb8                	sw	a4,88(a5)
            next->cbs.remaining_budget = next->cbs.budget;
    2054:	fa043783          	ld	a5,-96(s0)
    2058:	53f8                	lw	a4,100(a5)
    205a:	fa043783          	ld	a5,-96(s0)
    205e:	d7b8                	sw	a4,104(a5)
        }

        if(next->remaining_time > next->cbs.remaining_budget){
    2060:	fa043783          	ld	a5,-96(s0)
    2064:	4bf8                	lw	a4,84(a5)
    2066:	fa043783          	ld	a5,-96(s0)
    206a:	57bc                	lw	a5,104(a5)
    206c:	02e7d263          	bge	a5,a4,2090 <schedule_edf_cbs+0x43a>
            next->cbs.is_throttled=1;
    2070:	fa043783          	ld	a5,-96(s0)
    2074:	4705                	li	a4,1
    2076:	dbb8                	sw	a4,112(a5)
            next->cbs.throttle_new_deadline=next->current_deadline;
    2078:	fa043783          	ld	a5,-96(s0)
    207c:	4fb8                	lw	a4,88(a5)
    207e:	fa043783          	ld	a5,-96(s0)
    2082:	dfb8                	sw	a4,120(a5)
            next->cbs.remaining_time_at_throttle = next->remaining_time;
    2084:	fa043783          	ld	a5,-96(s0)
    2088:	4bf8                	lw	a4,84(a5)
    208a:	fa043783          	ld	a5,-96(s0)
    208e:	dff8                	sw	a4,124(a5)
        }
      
    }

    if (next != NULL) {
    2090:	fa043783          	ld	a5,-96(s0)
    2094:	c3a9                	beqz	a5,20d6 <schedule_edf_cbs+0x480>
        r.scheduled_thread_list_member = &next->thread_list;
    2096:	fa043783          	ld	a5,-96(s0)
    209a:	02878793          	addi	a5,a5,40
    209e:	f4f43023          	sd	a5,-192(s0)
        if(awating_rqe && preempt_thread_flag)
    20a2:	fa843783          	ld	a5,-88(s0)
    20a6:	cb91                	beqz	a5,20ba <schedule_edf_cbs+0x464>
    20a8:	fc842783          	lw	a5,-56(s0)
    20ac:	2781                	sext.w	a5,a5
    20ae:	c791                	beqz	a5,20ba <schedule_edf_cbs+0x464>
            r.allocated_time = time_untill_awaiting_thread;
    20b0:	fcc42783          	lw	a5,-52(s0)
    20b4:	f4f42423          	sw	a5,-184(s0)
    20b8:	a81d                	j	20ee <schedule_edf_cbs+0x498>
        else{
            // if we're here and next is throttled, the task will be throttled after this dispatch
            r.allocated_time = (next->cbs.is_throttled) ? next->cbs.remaining_budget : next->remaining_time;
    20ba:	fa043783          	ld	a5,-96(s0)
    20be:	5bbc                	lw	a5,112(a5)
    20c0:	c789                	beqz	a5,20ca <schedule_edf_cbs+0x474>
    20c2:	fa043783          	ld	a5,-96(s0)
    20c6:	57bc                	lw	a5,104(a5)
    20c8:	a021                	j	20d0 <schedule_edf_cbs+0x47a>
    20ca:	fa043783          	ld	a5,-96(s0)
    20ce:	4bfc                	lw	a5,84(a5)
    20d0:	f4f42423          	sw	a5,-184(s0)
    20d4:	a829                	j	20ee <schedule_edf_cbs+0x498>
            }

    } else {
        r.scheduled_thread_list_member = args.run_queue;
    20d6:	649c                	ld	a5,8(s1)
    20d8:	f4f43023          	sd	a5,-192(s0)
        r.allocated_time = (awating_rqe != NULL) ? time_untill_awaiting_thread : 1 ;
    20dc:	fa843783          	ld	a5,-88(s0)
    20e0:	c781                	beqz	a5,20e8 <schedule_edf_cbs+0x492>
    20e2:	fcc42783          	lw	a5,-52(s0)
    20e6:	a011                	j	20ea <schedule_edf_cbs+0x494>
    20e8:	4785                	li	a5,1
    20ea:	f4f42423          	sw	a5,-184(s0)

    }

   // printf("    second run\n");
   // __check_deadline_miss(args.run_queue,args.current_time+8);
    return r;
    20ee:	f4043783          	ld	a5,-192(s0)
    20f2:	f4f43823          	sd	a5,-176(s0)
    20f6:	f4843783          	ld	a5,-184(s0)
    20fa:	f4f43c23          	sd	a5,-168(s0)
    20fe:	4701                	li	a4,0
    2100:	f5043703          	ld	a4,-176(s0)
    2104:	4781                	li	a5,0
    2106:	f5843783          	ld	a5,-168(s0)
    210a:	893a                	mv	s2,a4
    210c:	89be                	mv	s3,a5
    210e:	874a                	mv	a4,s2
    2110:	87ce                	mv	a5,s3

    // handle the case where run queue is empty
    // TO DO

    return r;
}
    2112:	853a                	mv	a0,a4
    2114:	85be                	mv	a1,a5
    2116:	70ea                	ld	ra,184(sp)
    2118:	744a                	ld	s0,176(sp)
    211a:	74aa                	ld	s1,168(sp)
    211c:	790a                	ld	s2,160(sp)
    211e:	69ea                	ld	s3,152(sp)
    2120:	6129                	addi	sp,sp,192
    2122:	8082                	ret
